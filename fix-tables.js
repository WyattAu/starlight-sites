...
function walk(dir) {
  ...
  for (const d of fs.readdirSync(dir, { withFileTypes: true })) {
    ...
    if (d.isDirectory() && !['node_modules', 'dist', '.git'].includes(d.name))
      ...
      if (!inTable) {
        ...
        if (i + 1 < lines.length) {
          ...
          if (nextTrim.startsWith('|') && nextTrim.includes('---')) {
            // Has separator
            sepRow = i + 1
            const _sepCols = nextTrim.split('|').length - 1
        ...
        if (sepRow >= 0 && i !== sepRow) {
          ...
          if (rowCols > sepCols) {
            warnings.push(
              `${f}:${i + 1} - Row has ${rowCols} columns but separator has ${sepCols} columns: "${trimmed.substring(0, 80)}"`,
            )
          }
        }
      }
