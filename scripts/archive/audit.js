...
function scan(file) {
  ...
  const body = lines
    .slice(firstBody + 1)
    .filter(l => l.trim() !== '' && !l.trimStart().startsWith('import '))
  if (body.length === 0) {
    ISSUES.push({
      f: fn,
      l: 1,
...
function finalizeTable(fn, start, _cols, hasSep, rows) {
...
for (const iss of ISSUES) {
  process.stdout.write(
    `${iss.f}:${iss.l}:${iss.c} | ${iss.cat} | ${iss.sev.toUpperCase()} | ${iss.desc}\n`,
  )
}
