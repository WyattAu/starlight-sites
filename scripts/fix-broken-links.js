...
const sites = readdirSync(sitesDir).filter(s => {
  try { return statSync(join(sitesDir, s)).isDirectory() && s !== 'main'; } catch { return false; }
});
    ...
    try {
      ...
      for (let i = 0; i < lines.length; i++) {
        ...
        while ((match = linkRegex.exec(line)) !== null) {
          ...
          let cleanPath = pathPart.replace(/\.(md|mdx)$/, '');
          ...
          if (pathPart.startsWith('/') && !pathPart.startsWith('//')) {
            ...
            if (findExistingDocPath(siteDir, absPath)) {
              // Convert to relative
              const newRelPath = relative(fileDir, join(docsDir, absPath + '.md')).replace(/\.md$/, '');
          ...
          if (cleanPath.startsWith('../')) {
            ...
            if (ups <= fileParts.length) {
              ...
              if (findExistingDocPath(siteDir, tryPath)) {
                const newRelPath = relative(fileDir, join(docsDir, tryPath + '.md')).replace(/\.md$/, '');
                const newUrl = newRelPath + anchor;
                newLine = newLine.replace(fullMatch, '[' + linkText + '](' + newUrl + ')');
                modified = true;
                continue;
