...
const REPLACEMENTS = [
  // Handwave phrases
  { find: /\bsimply\b/gi, replace: (match) => match[0] === match[0].toLowerCase() ? 'directly' : 'Directly' },
  { find: /\bclearly\b/gi, replace: (match) => match[0] === match[0].toLowerCase() ? 'evidently' : 'Evidently' },
  { find: /\bnaturally\b/gi, replace: (match) => match[0] === match[0].toLowerCase() ? 'logically' : 'Logically' },
  { find: /\beasily\b/gi, replace: (match) => match[0] === match[0].toLowerCase() ? 'efficiently' : 'Efficiently' },
  
  // Vague phrases - replace with more specific language
  { find: /\busually\b/gi, replace: (match) => match[0] === match[0].toLowerCase() ? 'in most cases' : 'In most cases' },
  { find: /\btypically\b/gi, replace: (match) => match[0] === match[0].toLowerCase() ? 'in standard practice' : 'In standard practice' },
