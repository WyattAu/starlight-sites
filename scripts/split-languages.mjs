...
for (const lang of LANGUAGES) {
  ...
  for (const f of readdirSync(join(LANGUAGES_SITE, 'src', 'content', 'docs'), {
    withFileTypes: true,
  })) {
    if (
      f.isFile() &&
      (f.name.endsWith('.md') || f.name.endsWith('.mdx')) &&
      f.name.includes(lang.name)
    ) {
      cpSync(
        join(LANGUAGES_SITE, 'src', 'content', 'docs', f.name),
        join(siteDir, 'src', 'content', 'docs', f.name),
      )
    }
  }
  ...
  const config = template
    ...
    .replace(
      ...
      `site: 'https://${lang.name}.wyattau.com'`,
    )
  ...
  write(
    join(siteDir, 'src', 'content.config.ts'),
    read(join(LANGUAGES_SITE, 'src', 'content.config.ts')),
  )
