...
<aside aria-label="Polar coordinates appear in Edexcel FP2 and CIE P2. OCR (A) does not cover this topic. AQA" class="starlight-aside starlight-aside--note">
...
<aside aria-label="When converting from Cartesian to polar, always check the quadrant of the point. The" class="starlight-aside starlight-aside--caution">
...
<aside aria-label="A limacon $r = a + b\cos\theta$ has an inner loop if $b > a$A dimple if $a < b \leq 2a$" class="starlight-aside starlight-aside--tip">
...
<aside aria-label="The area formula uses $r^2$Not $r$. When computing the area between two curves, Subtract" class="starlight-aside starlight-aside--caution">
