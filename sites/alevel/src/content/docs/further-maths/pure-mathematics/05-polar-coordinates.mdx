...
<Aside type="note" title="Polar coordinates appear in Edexcel FP2 and CIE P2. OCR (A) does not cover this topic. AQA">
...
<Aside type="caution" title="When converting from Cartesian to polar, always check the quadrant of the point. The">
...
<Aside type="tip" title="A limacon $r = a + b\cos\theta$ has an inner loop if $b > a$A dimple if $a < b \leq 2a$">
...
<Aside type="caution" title="The area formula uses $r^2$Not $r$. When computing the area between two curves, Subtract">
