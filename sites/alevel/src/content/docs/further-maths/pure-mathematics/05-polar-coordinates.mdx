...
title: "Polar Coordinates | A-Level - Wyatt's Notes"
...
1. **Find key values:** Evaluate $r$ at $\theta = 0, \pi/6, \pi/4, \pi/3, \pi/2, \pi, 3\pi/2, 2\pi$.

2. **Find where $r = 0$:** These are points where the curve passes through the pole.

3. **Find maximum $|r|$:** Differentiate $r$ with respect to $\theta$ and set $dr/d\theta = 0$.

4. **Trace the curve:** As $\theta$ increases, plot the corresponding $(r, \theta)$ points and join
   them smoothly.
...
[Further Calculus](04-further-calculus).
...
Integration. See [Further Calculus](04-further-calculus).
...
```mermaid
flowchart TD
    A[05 Polar Coordinatesx] --> B[Key Concepts]
    A --> C[Core Principles]
