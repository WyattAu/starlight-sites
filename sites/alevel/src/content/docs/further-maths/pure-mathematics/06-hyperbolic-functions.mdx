...
{/* prettier-ignore */}
<DesmosGraph client:load title="Hyperbolic Functions: sinh, cosh, tanh" expressions={["\sinh(x)=(e^x-e^{-x})/2", "\cosh(x)=(e^x+e^{-x})/2", "\tanh(x)=\sinh(x)/\cosh(x)"]} width={800} height={500} />
