...
<Aside type="note" title="The formula booklet lists hyperbolic identities and the logarithmic forms of the inverse">
...
<Aside type="caution" title="Note the spelling: $\cosh$, $\sinh$, $\tanh$ are standard abbreviations. The reciprocals">
...
<Aside type="tip" title="Osborn\'s rule is a useful mnemonic but should not replace understanding. Always verify">
...
<Aside type="tip" title="When you encounter an integral of the form $\dfrac◆LB◆1◆RB◆◆LB◆\sqrt{x^2+a^2}◆RB◆$ or">
