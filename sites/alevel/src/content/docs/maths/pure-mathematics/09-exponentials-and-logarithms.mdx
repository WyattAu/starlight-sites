...
:::caution Warning $\frac{d}{dx}x^n = nx^{n-1}$ applies when the variable is in the **base**, not
the exponent.
:::
...
:::caution When substituting $u = a^x$ or $u = \ln x$Always check the domain. For $u = a^x$ we Have
