...
<Aside type="note" title="The formula booklet gives the laws of logarithms and the derivatives of $e^x$ and $\ln x$.">
...
<Aside type="caution" title="Warning $\frac{d}{dx}x^n = nx^{n-1}$ applies when the variable is in the **base**, not">
the exponent.
</Aside>
...
<Aside type="tip" title="Your calculator likely has $\log$ (base 10) and $\ln$ (base $e$) buttons, but not a general">
...
<Aside type="caution" title="When substituting $u = a^x$ or $u = \ln x$Always check the domain. For $u = a^x$ we Have">
...
<Aside type="caution" title="Warning Rule alone — it requires logarithmic differentiation or rewriting as">
$e^{g(x)\ln f(x)}$.
</Aside>
<hr />
...
<Aside type="tip" title="Tip Ready to test your understanding of **Exponentials and Logarithms**? The contains the">
