...
description: "Kinematics is the mathematical description of motion, without reference to the forces that cause it. We begin with three rigorous definitions."
...
:::tip
<strong>Exam Technique To decide which SUVAT equation to use, identify which variable is **not**</strong>
...
<PhetSimulation client:only="solid-js" simulationId="the-moving-man" title="The Moving Man" />
...
:::tip
<strong>Exam Technique When asked to find the distance travelled from a curved $v$-$t$ graph, check</strong>
...
:::
:::caution
<strong>Common Pitfall Students often draw a tangent to a curve incorrectly by placing the ruler</strong>
...
:::caution
<strong>Common Pitfall Terminal velocity does **not** mean the object has stopped accelerating</strong>
...
:::tip
<strong>Exam Technique Identify what the acceleration is given as a function of -- time, velocity, or</strong>
...
:::
:::caution
<strong>Common Pitfall The boat's speed $v_b$ is always relative to the water, not relative to</strong>
...
:::tip
<strong>Tip Ready to test your understanding of **Kinematics**? The contains the hardest questions</strong>
...
1. **Applying SUVAT equations to non-uniform acceleration.** The five SUVAT equations ONLY apply when acceleration is CONSTANT. If a question involves a changing force (e.g., a spring, air resistance), acceleration is not constant and SUVAT cannot be used. Use calculus (integration) or energy methods instead.

2. **Confusing displacement with distance.** Displacement is a VECTOR (includes direction) and can be negative. Distance is a SCALAR and is always positive. In vertical motion, upward displacement is positive and downward is negative. A ball thrown up and caught at the same point has zero total displacement but non-zero distance travelled.

3. **Taking g as positive in both directions.** When using SUVAT with vertical motion, be consistent with sign convention. If upward is positive, then g = -9.81 m/s² (acceleration is downward). Substituting g = +9.81 with upward-positive convention will give the wrong answer for velocity and time.

4. **Forgetting that air resistance changes the motion.** In many projectile questions, air resistance is neglected. But if a question mentions air resistance, remember: the horizontal component of velocity DECREASES (it is no longer constant), the time of flight DECREASES, and the trajectory is no longer a perfect parabola.

5. **Not resolving vectors into components before adding.** Velocity, displacement, and acceleration are vectors. You cannot add magnitudes directly unless the vectors are in the same direction. Always resolve into horizontal and vertical components first.


```mermaid
flowchart TD
    A[02 Kinematicsx] --> B[Key Concepts]
    A --> C[Core Principles]
...
- [Dynamics](./03-dynamics): Builds on kinematic descriptions by explaining the forces that cause motion and acceleration.
- [Work, Energy and Power](./04-work-energy-power): Connects velocity and displacement to energy concepts, showing how motion relates to energy transfer.
- [Projectile Motion](../../further-maths/further-mechanics/01-projectile-motion): Applies kinematic equations to objects launched with both horizontal and vertical components of motion.
- [Graphical Analysis](/physics/graphical-analysis): Explores how displacement-time and velocity-time graphs visually represent kinematic relationships.

---

## References

1. Giancoli, D. C. (2013). *Physics: Principles with Applications* (7th ed.). Pearson.
2. Halliday, D., Resnick, R. & Walker, J. (2013). *Fundamentals of Physics* (10th ed.). John Wiley & Sons.
3. Long, M. (2011). *Cambridge International AS and A Level Physics Coursebook*. Cambridge University Press.
4. Mee, C., Crundell, M., Brandstreet, A. & Brown, S. (2015). *Cambridge International AS & A Level Physics*. Hodder Education.
5. Young, H. D. & Freedman, R. A. (2019). *University Physics with Modern Physics* (15th ed.). Pearson.
