...
:::caution Common Pitfall In $\beta^-$ decay, the mass number $A$ does not change — only the atomic
...
:::
:::caution Warning Because: (1) the detector only intercepts a fraction of emitted radiation (solid
...
:::caution Warning Only when the parent is much longer-lived than all daughters. If a short-lived
