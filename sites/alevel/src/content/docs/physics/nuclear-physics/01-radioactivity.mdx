...
<Aside type="caution" title="Common Pitfall In $\beta^-$ decay, the mass number $A$ does not change — only the atomic">
...
<Aside type="tip" title="Tip Straight line with gradient $=-\lambda$. The half-life can be found by finding the time">
...
<Aside type="caution" title="Warning Because: (1) the detector only intercepts a fraction of emitted radiation (solid">
...
<Aside type="tip" title="Exam Technique When describing an experimental method for determining half-life, always">
...
<Aside type="note" title="Board Coverage AQA requires the graphical method; Edexcel and OCR (A) additionally require">
...
<Aside type="caution" title="Warning Only when the parent is much longer-lived than all daughters. If a short-lived">
...
<Aside type="note" title="Board Coverage The effective half-life is required by Edexcel and OCR (A) but is not">
...
<Aside type="tip" title="Exam Application When asked to evaluate the risks from a nuclear incident, consider: (1)">
...
<Aside type="tip" title="Diagnostic Test Ready to test your understanding of **Radioactivity**? The contains the hardest">
