...
<Aside type="tip" title="Exam Technique When calculating photon wavelengths from energy level transitions, first find">
...
<Aside type="caution" title="Warning Only $|\psi(x)|^2 = \psi^*(x)\psi(x)$ has physical meaning as a probability">
...
<Aside type="caution" title="Warning (nodes), but the probability of finding the particle in an **interval** of finite">
