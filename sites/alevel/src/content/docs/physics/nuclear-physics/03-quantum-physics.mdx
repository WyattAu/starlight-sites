...
<aside aria-label="Exam Technique When calculating photon wavelengths from energy level transitions, first find" class="starlight-aside starlight-aside--tip">
...
<Aside type="caution" title="Warning System." While this is a consequence, it is not the fundamental origin. The">
...
<Aside type="tip" title="Tip Confirms quantised energy levels." The key points are: (1) the periodic current drops">
...
<aside aria-label="Warning Only $|\psi(x)|^2 = \psi^*(x)\psi(x)$ has physical meaning as a probability" class="starlight-aside starlight-aside--caution">
...
<aside aria-label="Warning (nodes), but the probability of finding the particle in an **interval** of finite" class="starlight-aside starlight-aside--caution">
