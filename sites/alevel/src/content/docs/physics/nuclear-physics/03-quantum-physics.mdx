...
:::caution Warning System." While this is a consequence, it is not the fundamental origin. The
...
:::caution Warning Only $|\psi(x)|^2 = \psi^*(x)\psi(x)$ has physical meaning as a probability
...
:::caution Warning (nodes), but the probability of finding the particle in an **interval** of finite
