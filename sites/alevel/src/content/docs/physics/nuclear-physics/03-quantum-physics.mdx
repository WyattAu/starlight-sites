...
title: "Quantum Physics | A-Level - Wyatt's Notes"
...
<PhetSimulation client:only="solid-js" simulationId="photoelectric" title="Photoelectric Effect" />
...
1. The localisation of this wave packet is determined by the spread in $k$. For a Gaussian amplitude
   $A(k)$The resulting $\psi(x)$ is also Gaussian, and the widths satisfy:

$$\Delta x \cdot \Delta k = \frac{1}{2}$$

1. Since $p = \hbar k$We have $\Delta p = \hbar \, \Delta k$. Substituting:
...
1. Minimum kinetic energy (using $E_k = (\Delta p)^2 / 2m_e$):
...
1. **Radiation condition.** An electron in a stationary orbit does not radiate. It emits or absorbs
...
1. From the quantisation condition: $v = n\hbar/(m_e r)$.
...
1. Substituting $r_n = n^2\hbar^2/(m_e k e^2)$:
...
1. This is the simple harmonic oscillator equation with $k^2 = 2mE/\hbar^2$. The general solution
   is:
...
1. Applying the boundary condition $\psi(0) = 0$:

$$\psi(0) = A\sin(0) + B\cos(0) = B = 0 \implies B = 0$$

So $\psi(x) = A\sin(kx)$.

1. Applying the boundary condition $\psi(L) = 0$:
...
1. The normalised wave function (using $\int_0^L |\psi|^2 dx = 1$):
...
1. Total energy $E_n = -ke^2/(2r_n)$. For $n = 1$:
...
**Assuming intensity affects maximum kinetic energy:** In the photoelectric effect, intensity determines the number of photoelectrons (current), not their maximum kinetic energy. Maximum kinetic energy depends only on frequency and work function: $E_{k,max} = hf - \phi$.

**Misapplying the de Broglie wavelength formula:** The formula $\lambda = h/p$ applies to all matter, but students often forget to use the correct momentum. For non-relativistic particles, $p = mv$. For photons, $p = E/c = hf/c$. Don't use $p = mc$ for massive particles.

**Confusing energy levels with energy differences:** The energy levels $E_n = -13.6/n^2$ eV give the energy of a state. The energy of an emitted photon is the *difference* between two levels: $\Delta E = E_{n_i} - E_{n_f}$. Students often forget to subtract and use $E_n$ directly.

**Forgetting that $n = 0$ is not allowed in the infinite well:** The quantum number starts at $n = 1$ for the infinite potential well. $n = 0$ gives $\psi = 0$ everywhere (no particle exists). This is different from the harmonic oscillator, where $n = 0$ is allowed and gives the ground state.


```mermaid
flowchart TD
    A[03 Quantum Physicsx] --> B[Key Concepts]
    A --> C[Core Principles]
...
2. **Assuming intensity affects maximum kinetic energy.** In the photoelectric effect, intensity determines the number of photoelectrons (current), not their maximum kinetic energy. Maximum kinetic energy depends only on frequency and work function: $E_{k,\max} = hf - \phi$.

3. **Misapplying the de Broglie wavelength formula.** The formula $\lambda = h/p$ applies to all matter, but students often forget to use the correct momentum. For non-relativistic particles, $p = mv$. For photons, $p = E/c = hf/c$. Don't use $p = mc$ for massive particles.

4. **Confusing energy levels with energy differences.** The energy levels $E_n = -13.6/n^2$ eV give the energy of a state. The energy of an emitted photon is the *difference* between two levels: $\Delta E = E_{n_i} - E_{n_f}$. Students often forget to subtract and use $E_n$ directly.

5. **Forgetting that n = 0 is not allowed in the infinite well.** The quantum number starts at $n = 1$ for the infinite potential well. $n = 0$ gives $\psi = 0$ everywhere (no particle exists). This is different from the harmonic oscillator, where $n = 0$ is allowed and gives the ground state.

## Cross-References

- [Radioactivity](./01-radioactivity.mdx): Connects quantum mechanical principles to the random nature of radioactive decay and nuclear transitions.
- [Nuclear Energy](./02-nuclear-energy.mdx): Shows how mass-energy equivalence and quantum effects enable fission and fusion reactions.
- [Wave-Particle Duality](/physics/wave-particle-duality): Explores the de Broglie hypothesis and evidence for wave behaviour of particles in more detail.
- [Electron Configuration](/physics/electron-configuration): Applies quantum numbers and orbital theory to explain atomic structure and chemical properties.
