...
<Aside type="caution" title="Warning Work done **by** the system. Always check which convention is being used. We use">
...
<Aside type="caution" title="Common Pitfall Students often confuse engine efficiency ($\eta = W/Q_H$) with">
...
<Aside type="tip" title="Tip Give a consequence (no 100% efficient engine), and explain in terms of entropy if">
