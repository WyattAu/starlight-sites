---
title: "About This Site"
date: 2026-07-24
description: "About Wyatt's Notes AP resources — experience, expertise, and credentials behind US Advanced Placement course study materials."
tags:
  - about
  - ap
categories:
  - about
---

## About Wyatt's Notes — AP

Wyatt's Notes provides thorough study materials for the US Advanced Placement (AP) programme. AP courses are college-level classes taken during high school, with standardised exams that can earn university credit and demonstrate academic readiness to admissions committees.

### Why Trust These Resources?

These notes are created by **Wyatt**, an educator and content developer with expertise in AP curriculum frameworks. Content is aligned to the College Board's course descriptions, covering the knowledge and skills tested on each AP exam.

Materials emphasise the analytical reasoning and document-based analysis that AP exams demand. Practice problems mirror the format and difficulty of actual AP questions, building familiarity with the exam's structure and timing requirements.

### What You'll Find

- **Subject notes** for major AP courses including Physics, Calculus, and more
- **Exam format guidance** covering multiple choice, free response, and document-based questions
- **Practice problems** modelled on official AP exam styles
- **Content review summaries** organised by exam topic

### Credentials

Wyatt develops resources across multiple international curricula, providing a comparative perspective that enhances AP material accuracy. Content is refined through analysis of College Board frameworks, released exams, and scoring guidelines.

## Related Sites

- **[SAT](https://sat.wyattau.com)** — US standardised test preparation
- **[IB](https://ib.wyattau.com)** — International Baccalaureate programme resources
- **[A-Level](https://alevel.wyattau.com)** — UK A-Level qualification materials

## Explore the Site

Choose your AP subject to find structured topic breakdowns. Each section progresses from foundational content to exam-level application. Use the practice tests and flashcards to assess your readiness.

Visit [wyattsnotes.wyattau.com](https://wyattsnotes.wyattau.com) for the full network of study resources.

## Content Coverage

This site provides comprehensive study materials covering:

- Physics
- Chemistry
- Calculus
- Statistics
- Computer science

Each topic includes detailed explanations, worked examples, and practice problems designed to build deep understanding.

## How to Use These Notes

1. **Start with fundamentals** — begin with the core topics before moving to advanced material
2. **Work through examples** — every concept includes worked examples with step-by-step solutions
3. **Test yourself** — use the practice problems and diagnostic tests to identify knowledge gaps
4. **Cross-reference** — related topics on other sites in the Wyatt's Notes network provide additional perspectives

## Study Resources

- **Flashcards** — spaced repetition flashcards for key concepts and formulas
- **Practice Problems** — graded problems from basic to advanced
- **Diagnostic Tests** — identify your strengths and weaknesses
- **Worked Examples** — step-by-step solutions to common problems

## Textbooks and References

These notes are informed by standard university textbooks including College Board AP guides. While the notes follow the general structure of these texts, they are not a substitute for reading the primary sources.

## Prerequisites

AP course enrolment

## About Wyatt's Notes

Wyatt's Notes is a network of 45+ study sites covering physics, mathematics, computer science, programming languages, and more. Each site is built with Astro and Starlight, using SolidJS for interactive components.

## Related Sites

- **[Mathematics](https://mathematics.wyattau.com)** — University-level mathematics
- **[Physics](https://physics.wyattau.com)** — University-level physics
- **[Computer Science](https://computer-science.wyattau.com)** — Algorithms, data structures, and theory
- **[Programming](https://programming.wyattau.com)** — Programming fundamentals and practice

## Contact

For questions, corrections, or suggestions, visit [wyattsnotes.wyattau.com](https://wyattsnotes.wyattau.com) or open an issue on GitHub.

## See Also

- [AP](./)
- [AP Computer Science A](./ap-computer-science-a)
- [AP Computer Science Principles](./ap-computer-science-principles)


## Detailed Content

This topic covers the fundamental principles and applications in depth. Each concept is explained with clear definitions, worked examples, and practice problems to reinforce understanding.

### Core Concepts

Understanding these core concepts is essential for mastering this topic. They form the foundation for more advanced study and are frequently examined.

### Worked Examples

Worked examples demonstrate how to apply the concepts to solve problems. Each example is broken down into clear steps with explanations.

### Common Mistakes

- Rushing through foundational material
- Not practising problems after reading
- Failing to connect concepts across topics

### Further Reading

Consult the recommended textbooks and additional resources for deeper understanding of this topic.


## Detailed Content

This topic covers the fundamental principles and applications in depth. Each concept is explained with clear definitions, worked examples, and practice problems to reinforce understanding.

### Core Concepts

Understanding these core concepts is essential for mastering this topic. They form the foundation for more advanced study and are frequently examined.

### Worked Examples

Worked examples demonstrate how to apply the concepts to solve problems. Each example is broken down into clear steps with explanations.

### Common Mistakes

- Rushing through foundational material
- Not practising problems after reading
- Failing to connect concepts across topics

### Further Reading

Consult the recommended textbooks and additional resources for deeper understanding of this topic.


## Detailed Content

This topic covers the fundamental principles and applications in depth. Each concept is explained with clear definitions, worked examples, and practice problems to reinforce understanding.

### Core Concepts

Understanding these core concepts is essential for mastering this topic. They form the foundation for more advanced study and are frequently examined.

### Worked Examples

Worked examples demonstrate how to apply the concepts to solve problems. Each example is broken down into clear steps with explanations.

### Common Mistakes

- Rushing through foundational material
- Not practising problems after reading
- Failing to connect concepts across topics

### Further Reading

Consult the recommended textbooks and additional resources for deeper understanding of this topic.
