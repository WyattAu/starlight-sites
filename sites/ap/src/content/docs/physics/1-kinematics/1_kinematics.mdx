...
Kinematics is the **language of motion** — it describes _what_ is happening without asking _why_. Think of it as the GPS of physics: it tells you position, velocity, and acceleration, but not the forces causing them.

**The calculus connection:** Velocity is the derivative of position, acceleration is the derivative of velocity. This isn't just a formula — it's the _definition_. The kinematic equations for constant acceleration are directly what you get when you integrate $a = \text{const}$ once (to get $v$) and twice (to get $x$). If acceleration isn't constant, you must use calculus directly.

**Projectile motion intuition:** The horizontal and vertical motions are completely independent. Gravity only acts vertically, so the horizontal velocity never changes. This means you can analyse each direction separately using the same equations — a powerful simplification.

**Circular motion intuition:** Even at constant speed, an object moving in a circle is accelerating — because acceleration is the rate of change of the _velocity vector_, and the direction keeps changing. This centripetal acceleration always points toward the center, like a tether pulling the object inward.
...
1. **Assuming that the magnitude of acceleration equals $g$ for objects thrown upward.** The
...
2. **Using the wrong reference frame in relative motion problems.** Always identify the observer's
    frame and the moving frame explicitly. Write the velocity addition formula carefully.

3. **Forgetting that in 2D projectile motion, the time determines everything.** Once you find the
...
4. **Confusing angular displacement with linear displacement in circular motion problems.** Angular
    displacement $\theta$ is dimensionless (radians). Linear displacement $s = r\theta$ has units of
    metres.

5. **Assuming that $a = v^2/r$ applies when the speed is not constant.** The formula $a_c = v^2/r$
...
1. A particle moves along the $x$-axis with acceleration $a(t) = 8t - 6$ m/s$^2$. At $t = 0$The
...
2. A projectile is launched from ground level at $40 \mathrm{ m/s$. Find the two launch angles that
    give a range of $120 \mathrm{ m$.

3. A car accelerates from rest at $3 \mathrm{ m/s^2$ for $4 \mathrm{ s$ Then travels at constant
...
4. Two boats leave the same point simultaneously. Boat A heads north at $6 \mathrm{ m/s$ and Boat B
    heads northeast at $8 \mathrm{ m/s$. Find the velocity of Boat A relative to Boat B.

5. An astronaut on the Moon throws a ball vertically upward at $15 \mathrm{ m/s$. Find the maximum
...
:::note
<strong>Even though the particle is stationary at $t = 3 \mathrm{ s$The acceleration is also zero</strong>
...
1. A ball is thrown upward from the top of a $50 \mathrm{ m$ building at $20 \mathrm{ m/s$.
...
2. A police car chasing a speeding motorist. The motorist passes at $30 \mathrm{ m/s$. The police
...
3. A projectile is launched from a cliff of height $80 \mathrm{ m$ at $50 \mathrm{ m/s$ at
...
4. Two trains approach each other on parallel tracks. Train A travels at $60 \mathrm{ m/s$ and
...
5. The position of a particle is given by $x(t) = 2t^3 - 9t^2 + 12t + 1$ (m). Find (a) the times
...
- [Newton's Laws](../2-newtons-laws/2_newtons-laws) -- Forces cause acceleration, connecting kinematic descriptions of motion to the dynamics that explain why objects move.
- [Work, Energy, and Power](../3-work-energy-power/3_work-energy-power) -- Kinematic equations derive from energy conservation, linking motion analysis to energy methods.
- [Momentum and Impulse](../4-momentum-and-impulse/4_momentum-and-impulse) -- Impulse equals change in momentum, connecting force-time analysis to the kinematic outcomes.
- **[AP Calculus — Derivatives](../../maths/2-derivatives/2_derivatives):** Velocity and acceleration are derivatives of position, making differentiation the mathematical tool for instantaneous rates of change in motion.

## References

1. Young, H. D. & Freedman, R. A. (2019). *University Physics with Modern Physics* (15th ed.). Pearson.
...
4. Hewitt, P. G. (2015). *Conceptual Physics* (12th ed.). Pearson.
