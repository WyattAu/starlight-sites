...
title: "Newton's Laws of Motion | AP - Wyatt's Notes"
...
```mermaid
flowchart TD
    A[2_Newtons Lawsx] --> B[Key Concepts]
    A --> C[Core Principles]
...
Newton's laws answer the question **why do objects move the way they do?** The key insight is that forces cause _changes_ in motion, not motion itself. An object moving at constant velocity needs no net force — it's the _acceleration_ that requires a force.
...
**Second Law intuition:** $\vec{F} = m\vec{a}$ is not a definition — it's an empirical law. Force is defined independently through interactions (gravity, springs, contact). The deep fact is that the same proportionality constant ($m$) works for _all_ forces. This is why we can predict motion from forces and infer forces from motion.

**Third Law intuition:** Forces always come in pairs acting on _different objects_. When you push a wall, the wall pushes back on you with equal force. But these forces don't cancel because they act on different bodies. This is why a book on a table doesn't accelerate — not because $\vec{N}$ and $\vec{W}$ are an action-reaction pair (they aren't), but because they act on the _same_ object and happen to balance.

**Free-body diagrams:** The single most important skill in mechanics. Isolate the object, draw every force acting _on_ it, and never include forces the object exerts on other things. Get this right, and the rest is algebra.
...
1. **Assuming the normal force equals $mg$ on an inclined plane.** The normal force is
    $mg\cos\theta$Not $mg$. Always resolve forces perpendicular to the surface.

2. **Forgetting that tension has a maximum value.** A string can only pull, not push. If the
    calculated tension is negative, the string has gone slack and the tension is zero.

3. **Applying $F = ma$ to individual objects when using the system approach.** The system approach
    gives the acceleration of the entire system but does not give tensions or internal forces.

4. **Ignoring air resistance in orbital calculations.** While this is a valid approximation for
    satellites above the atmosphere, it fails for objects in the lower atmosphere.

5. **Using $g = 9.8 \mathrm{ m/s^2$ for objects far from Earth\'s surface.** Use $g = GM/r^2$ with
    the actual distance from Earth\'s centre.

6. **Confusing the mass of an orbiting body with the mass of the central body.** In
    $v = \sqrt{GM/r}$, $M$ is the mass of the body being orbited (e.g., Earth), not the satellite.

## Practice Questions (Additional)

1. A block of mass $m$ is placed on a wedge of mass $M$ with angle $\theta$. All surfaces are
    frictionless. Find the acceleration of the wedge. (Hint: use conservation of momentum in the
    horizontal direction.)

2. A $3 \mathrm{ kg$ block on a frictionless table is connected by a string over a pulley to a
...
3. A rocket of mass $1000 \mathrm{ kg$ is launched vertically. The engine produces a thrust of
...
4. Two masses $m_1 = 2 \mathrm{ kg$ and $m_2 = 3 \mathrm{ kg$ are connected by a light string over
    a smooth pulley. $m_1$ rests on a rough table with $\mu_k = 0.3$. Find the acceleration and the
    tension in the string.

5. Calculate the gravitational field strength at a point halfway between the Earth and the Moon.
...
1. A $3 \mathrm{ kg$ block on a $25^\circ$ incline is connected by a string over a pulley to a
...
2. A helicopter of mass $3000 \mathrm{ kg$ is rising at $2 \mathrm{ m/s^2$. Calculate the upward
    force from the rotors. If the helicopter then moves at constant velocity, what upward force is
    needed?

3. A block of mass $m$ is placed on a frictionless cone with half-angle $\theta$. The cone rotates
...
4. A $10 \mathrm{ kg$ crate is pushed across a rough floor ($\mu_k = 0.4$) by a force of
    $80 \mathrm{ N$ applied at $30^\circ$ below the horizontal. Calculate the normal force, the
    friction force, and the acceleration.

5. Two blocks ($m_1 = 3 \mathrm{ kg$ on a table, $m_2 = 2 \mathrm{ kg$ hanging) are connected by a
...
- **[AP Calculus — Differential Equations](../../maths/4-differential-equations/4_differential-equations):** Newton's second law F = ma is a second-order differential equation; solving it requires integration to find velocity and position from acceleration.
