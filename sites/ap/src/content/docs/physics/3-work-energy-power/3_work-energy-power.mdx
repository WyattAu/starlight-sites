...
title: "Work, Energy, and Power | AP - Wyatt's Notes"
description: "Work is the energy transferred to or from an object by a force acting through a  Comprehensive educational content coverage with definitions and practice proble"
date: 2026-04-14
tags:
  - ap
  - ap-physics
...
:::
:::note
<strong>Example: With Friction</strong>
...
```mermaid
flowchart TD
    A[3_Work Energy Powerx] --> B[Key Concepts]
    A --> C[Core Principles]
...
Work, energy, and power provide a **scalar alternative to vector-based force analysis**. Instead of tracking forces and accelerations, you track energy — a single number that captures the state of the system. The work-energy theorem is the bridge: net work done on an object equals its change in kinetic energy.

**Conservation of energy intuition:** Energy cannot be created or destroyed, only transformed. In an isolated system, the total energy is constant. This is more powerful than Newton's laws in many situations because it's a scalar equation (no components to track) and works even when forces are variable or unknown.

**Power intuition:** Power is the _rate_ of energy transfer — how fast you do work. Two cars can deliver the same work (same KE change), but the sports car does it in less time (higher power). On the AP exam, problems often ask "how long does it take?" which is a power question, not an energy question.

**Non-conservative forces:** Friction and air resistance dissipate energy as heat. When non-conservative forces act, mechanical energy is not conserved — you must account for the energy lost to heat. The work-energy theorem still applies, but you include the work done by non-conservative forces.
...
1. **Using the wrong sign for work done by gravity.** When an object moves down, gravity does
...
2. **Confusing the reference level for gravitational PE.** The choice of $h = 0$ is arbitrary. What
    matters is the _change_ in height, $\Delta h$Not the absolute height.

3. **Applying $U = mgh$ when the height change is a significant fraction of the distance from
    Earth\'s centre.** Use $U = -GMm/r$ for large altitude changes.

4. **Forgetting that power is the time derivative of work, not force.** $P = dW/dt = Fv$. Power is
    not $F/t$ or $F/t^2$.

5. **Assuming the force is constant when using $W = Fd\cos\theta$.** If the force varies with
    position, you must integrate: $W = \int F(x)\, dx$.

## Practice Questions (Additional)

1. A $1.5 \mathrm{ kg$ object slides from rest down a curved frictionless ramp of height
    $3 \mathrm{ m$ and enters a rough horizontal section with $\mu_k = 0.4$. How far does it slide
    on the rough section before stopping?

2. A spring with $k = 800 \mathrm{ N/m$ is compressed $0.10 \mathrm{ m$ and launches a
    $0.2 \mathrm{ kg$ ball vertically. Find the maximum height reached (a) on Earth and (b) on the
    Moon ($g = 1.62 \mathrm{ m/s^2$).

3. A force $F(x) = -kx + bx^3$ acts on a particle. Find the potential energy function $U(x)$ and
    identify the equilibrium positions.

4. A $50 \mathrm{ kg$ child on a swing is pushed to a height $0.8 \mathrm{ m$ above the lowest
    point. Find the speed at the lowest point and the speed when the swing is $0.4 \mathrm{ m$ above
    the lowest point.

5. The potential energy of a particle is $U(r) = \frac{A}{r} - \frac{B}{r^2}$ where $A$ and $B$ are
...
:::note
<strong>Info Air density, $C_D$ is the drag coefficient, and $A$ is the cross-sectional area. This</strong>
...
1. A $1200 \mathrm{ kg$ car accelerates from $0$ to $30 \mathrm{ m/s$ in $8 \mathrm{ s$. Calculate
...
2. A $0.5 \mathrm{ kg$ ball is dropped from a height of $20 \mathrm{ m$. On each bounce, it loses
...
3. A force $F = 10 + 3x$ (N) acts on a $4 \mathrm{ kg$ object from $x = 0$ to $x = 5 \mathrm{ m$.
...
4. A roller coaster car of mass $500 \mathrm{ kg$ starts from rest at point A, height
...
5. Two springs with $k_1 = 200 \mathrm{ N/m$ and $k_2 = 300 \mathrm{ N/m$ are connected (a) in
...
- **[Kinematics](../1-kinematics/1_kinematics):** Work-energy theorem connects force and displacement to changes in speed without needing time.
- **[Momentum and Impulse](../4-momentum-and-impulse/4_momentum-and-impulse):** Work-energy and impulse-momentum are complementary approaches — energy for scalar problems, momentum for vector problems.
- **[Newton's Laws](../2-newtons-laws/2_newtons-laws):** Newton's second law is the foundation; work-energy is its integral form.
- **[AP Calculus — Integrals](../../maths/3-integrals/3_integrals):** Work is defined as the integral of force over displacement, making integration the mathematical foundation of energy analysis.
