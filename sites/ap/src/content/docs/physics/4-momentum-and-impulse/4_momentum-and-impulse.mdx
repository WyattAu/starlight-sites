...
description: "Momentum and Impulse: comprehensive educational content notes with precise definitions, worked examples, common pitfalls, and practice problems."
date: 2026-04-14
tags:
  - ap
  - ap-physics
...
<PhetSimulation client:only="solid-js" simulationId="collision-lab" title="Collision Lab" />
...
```mermaid
flowchart TD
    A[4_Momentum And Impulsex] --> B[Key Concepts]
    A --> C[Core Principles]
...
Momentum is the **conserved quantity of motion** — and it's conserved more often than energy. When two objects collide, internal forces are enormous but brief, so momentum is approximately conserved even when kinetic energy is not (as in inelastic collisions).

**Impulse intuition:** Impulse is the *area under the force-time graph*. A small force applied for a long time can produce the same velocity change as a large force applied briefly. This is why airbags work: they increase the collision time, reducing the peak force on your body.

**Why momentum is more fundamental than force:** $F = ma$ is a special case of $F = dp/dt$. For rockets (changing mass), raindrops (accumulating mass), or relativistic particles, only the momentum form works. Momentum is conserved; force is not.

**Elastic vs inelastic collisions:** In elastic collisions, both momentum and kinetic energy are conserved — think billiard balls. In inelastic collisions, momentum is conserved but KE is converted to heat, sound, or deformation. In *perfectly* inelastic collisions, the objects stick together and KE loss is maximized.

**Center of mass:** The center of mass of a system moves as if all external forces were applied to a single particle. Internal forces don't affect the CM motion — this is why a fireworks shell explodes symmetrically in mid-air.
...
1. A $0.05 \mathrm{ kg$ tennis ball hits a racket at $20 \mathrm{ m/s$ and rebounds at
...
2. A $5 \mathrm{ kg$ object moving at $8 \mathrm{ m/s$ collides with a $3 \mathrm{ kg$ object
...
3. A force $F(t) = 12t - 3t^2$ (in N) acts on a $4 \mathrm{ kg$ object initially moving at
...
4. Two ice skaters of masses $50 \mathrm{ kg$ and $70 \mathrm{ kg$ stand facing each other on
...
5. A rocket burns fuel at a rate of $100 \mathrm{ kg/s$ with an exhaust velocity of
...
:::note
<strong>Info Make the block complete full vertical circles. In most textbook versions, the block</strong>
...
*area under the force-time graph*, not just the peak force.

### Pitfall 8: Assuming KE Is Conserved in All Collisions

Only *elastic* collisions conserve kinetic energy. In real-world collisions, some KE is always
...
1. A $1500 \mathrm{ kg$ car travelling at $20 \mathrm{ m/s$ rear-ends a $2500 \mathrm{ kg$ truck
...
2. A tennis ball of mass $0.06 \mathrm{ kg$ is struck by a racket. The force on the ball during the
...
3. A $3 \mathrm{ kg$ object moving at $5 \mathrm{ m/s$ collides elastically with a $1 \mathrm{ kg$
    object at rest. Use the elastic collision formulas to find the final velocities of both objects.

4. A $60 \mathrm{ kg$ person standing on ice catches a $2 \mathrm{ kg$ ball thrown at
...
5. Two identical objects collide. Object A has velocity $3\hat{i} + 2\hat{j}$ m/s and Object B has
...
- **[Kinematics](../1-kinematics/1_kinematics):** Momentum conservation applies during collisions — kinematics describes the motion before and after.
- **[Work, Energy, and Power](../3-work-energy-power/3_work-energy-power):** Energy is conserved in elastic collisions; momentum is always conserved.
- **[Newton's Laws](../2-newtons-laws/2_newtons-laws):** Impulse is the integral of force over time — Newton's second law in momentum form.
- **[AP Calculus — Integrals](../../maths/3-integrals/3_integrals):** Impulse is the integral of force over time, connecting the impulse-momentum theorem to integral calculus.
