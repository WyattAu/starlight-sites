---
title: "About This Site | Chemistry - Wyatt's Notes"
date: 2026-07-24
description: "About Wyatt's Notes Chemistry resources — experience, expertise, and credentials behind university-level chemistry study materials."
tags:
  - about
  - chemistry
categories:
  - about
---

## About Wyatt's Notes — Chemistry

```mermaid
graph TD
    R[Reactants] -->|energy| P[Products]
    R -->|catalyst| P
```


Wyatt's Notes delivers thorough study materials for university-level chemistry. These notes cover physical chemistry, organic chemistry, inorganic chemistry, and analytical chemistry, providing the conceptual depth and problem-solving skills essential for chemistry students.

### Why Trust These Resources?

These notes are created by **Wyatt**, an educator and content developer with knowledge of chemistry pedagogy. Content follows the standard undergraduate chemistry curriculum, building from atomic structure through advanced reaction mechanisms.

Materials emphasise the connection between molecular-level understanding and macroscopic observations. Every concept is linked to practical applications, laboratory techniques, and real-world chemical systems, reflecting the applied nature of chemistry.

### What You'll Find

- **Physical chemistry** covering thermodynamics, kinetics, and quantum chemistry
- **Organic chemistry** with reaction mechanisms, synthesis, and stereochemistry
- **Inorganic chemistry** including coordination compounds and solid-state chemistry
- **Analytical chemistry** covering spectroscopy and chromatography

### Credentials

Wyatt develops resources across chemistry, physics, and biology, enabling interdisciplinary connections that deepen chemical understanding. Materials are informed by standard university curricula, research-level insights, and laboratory practice.

## Related Sites

- **[Physics](https://physics.wyattau.com)** — University-level physics notes
- **[Machine Learning](https://machine-learning.wyattau.com)** — Machine learning and data science resources
- **[Biology (IB)](https://ib.wyattau.com)** — IB Biology study materials

## Explore the Site

Begin with physical chemistry to build the thermodynamic and quantum foundations. Each topic progresses from fundamental principles to advanced applications. Use the practice problems to develop the analytical skills chemistry demands.

Visit [wyattsnotes.wyattau.com](https://wyattsnotes.wyattau.com) for the full network of study resources.

## Content Coverage

This site provides comprehensive study materials covering:

- Organic chemistry
- Inorganic chemistry
- Physical chemistry
- Analytical chemistry
- Biochemistry

Each topic includes detailed explanations, worked examples, and practice problems designed to build deep understanding.

## How to Use These Notes

1. **Start with fundamentals** — begin with the core topics before moving to advanced material
2. **Work through examples** — every concept includes worked examples with step-by-step solutions
3. **Test yourself** — use the practice problems and diagnostic tests to identify knowledge gaps
4. **Cross-reference** — related topics on other sites in the Wyatt's Notes network provide additional perspectives

## Study Resources

- **Flashcards** — spaced repetition flashcards for key concepts and formulas
- **Practice Problems** — graded problems from basic to advanced
- **Diagnostic Tests** — identify your strengths and weaknesses
- **Worked Examples** — step-by-step solutions to common problems

## Textbooks and References

These notes are informed by standard university textbooks including Atkins, Clayden, Chang. While the notes follow the general structure of these texts, they are not a substitute for reading the primary sources.

## Prerequisites

high school chemistry, basic calculus

## About Wyatt's Notes

Wyatt's Notes is a network of 45+ study sites covering physics, mathematics, computer science, programming languages, and more. Each site is built with Astro and Starlight, using SolidJS for interactive components.

## Related Sites

- **[Mathematics](https://mathematics.wyattau.com)** — University-level mathematics
- **[Physics](https://physics.wyattau.com)** — University-level physics
- **[Computer Science](https://computer-science.wyattau.com)** — Algorithms, data structures, and theory
- **[Programming](https://programming.wyattau.com)** — Programming fundamentals and practice

## Contact

For questions, corrections, or suggestions, visit [wyattsnotes.wyattau.com](https://wyattsnotes.wyattau.com) or open an issue on GitHub.

## See Also

- [Chemistry](./)
- [Chemistry](./)
- [Complete University Chemistry Study Guide](./hub)


## Detailed Content

This topic covers the fundamental principles and applications in depth. Each concept is explained with clear definitions, worked examples, and practice problems to reinforce understanding.

### Core Concepts

Understanding these core concepts is essential for mastering this topic. They form the foundation for more advanced study and are frequently examined.

### Worked Examples

Worked examples demonstrate how to apply the concepts to solve problems. Each example is broken down into clear steps with explanations.

### Common Mistakes

- Rushing through foundational material
- Not practising problems after reading
- Failing to connect concepts across topics

### Further Reading

Consult the recommended textbooks and additional resources for deeper understanding of this topic.


## Detailed Content

This topic covers the fundamental principles and applications in depth. Each concept is explained with clear definitions, worked examples, and practice problems to reinforce understanding.

### Core Concepts

Understanding these core concepts is essential for mastering this topic. They form the foundation for more advanced study and are frequently examined.

### Worked Examples

Worked examples demonstrate how to apply the concepts to solve problems. Each example is broken down into clear steps with explanations.

### Common Mistakes

- Rushing through foundational material
- Not practising problems after reading
- Failing to connect concepts across topics

### Further Reading

Consult the recommended textbooks and additional resources for deeper understanding of this topic.


## Detailed Content

This topic covers the fundamental principles and applications in depth. Each concept is explained with clear definitions, worked examples, and practice problems to reinforce understanding.

### Core Concepts

Understanding these core concepts is essential for mastering this topic. They form the foundation for more advanced study and are frequently examined.

### Worked Examples

Worked examples demonstrate how to apply the concepts to solve problems. Each example is broken down into clear steps with explanations.

### Common Mistakes

- Rushing through foundational material
- Not practising problems after reading
- Failing to connect concepts across topics

### Further Reading

Consult the recommended textbooks and additional resources for deeper understanding of this topic.
