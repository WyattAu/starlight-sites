...
<script type="application/ld+json">
{
  ...
  "@type": "BreadcrumbList",
  "itemListElement": [{"name": "Home", "url": "https://wyattau.com"}, {"name": "cpp", "url": "https://cpp.wyattau.com"}, {"name": "Compilation_model", "url": "https://cpp.wyattau.com/compilation_model"}, {"name": "1_translation", "url": "https://cpp.wyattau.com/compilation_model/1_translation"}, {"name": "4_binary_formats", "url": "https://cpp.wyattau.com/compilation_model/1_translation/4_binary_formats"}]
}
</script>
title: Binary Formats
description: "When the linker completes its work, it produces a binary file. This file is not merely a linear Sequence of machine code; it is a structured database"
...
- [The C Runtime (CRT)](../2_modules/4_c_runtime.mdx)
## Summary
...
Binary formats — ELF on Linux, PE on Windows, Mach-O on macOS — are the final output of the compilation process and the input to the operating system's loader. At its core, a binary format is a **map for the OS**: it tells the loader which parts of the file are code, which are read-only data, which are writable, and how to relocate addresses when the binary is loaded at a non-preferred address (ASLR). Understanding this map means you can diagnose linking errors, understand why certain memory accesses cause segfaults, and optimise binary size by knowing what each section contains.

The three formats share the same fundamental structure — headers describing sections, sections containing code and data, and symbol tables for linking — but differ in implementation details. ELF uses program headers and section headers with a rich set of metadata sections (`.eh_frame` for unwinding, `.dynamic` for dynamic linking). PE uses a different directory-based system with import/export tables and TLS directories. Mach-O uses load commands and requires code signing on Apple platforms. The differences matter when cross-compiling, debugging, or analysing binaries.

The intuition to build is that **every byte in a binary has a purpose and a permission**. The `.text` section is executable but not writable; `.rodata` is readable but not writable or executable; `.data` is readable and writable. These permissions are enforced by the hardware (via page table entries) and the OS. Security features like DEP/NX, ASLR, and stack canaries all operate at this level. When you strip debug symbols, you are removing `.debug_*` sections that are not loaded at runtime. When you see a segfault, the first step is to check which segment the faulting address belongs to — this tells you whether it is a code, data, or stack issue.