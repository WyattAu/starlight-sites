...
<script type="application/ld+json">
{
  ...
  "@type": "BreadcrumbList",
  "itemListElement": [{"name": "Home", "url": "https://wyattau.com"}, {"name": "cpp", "url": "https://cpp.wyattau.com"}, {"name": "Compilation_model", "url": "https://cpp.wyattau.com/compilation_model"}, {"name": "2_modules", "url": "https://cpp.wyattau.com/compilation_model/2_modules"}, {"name": "4_c_runtime", "url": "https://cpp.wyattau.com/compilation_model/2_modules/4_c_runtime"}]
}
</script>
...
The C Runtime (CRT) is the invisible scaffolding that makes a C++ program run. Before your `main` function is called, the CRT has already done significant work: it set up the stack, initialised the heap, processed global constructors (`.init_array`), and set up thread-local storage. When `main` returns, the CRT runs `atexit` handlers, calls static destructors, flushes I/O buffers, and then invokes the OS exit syscall. Understanding this lifecycle is critical because bugs in global constructors, static destructors, or heap initialisation happen *before your code even runs* — and they are notoriously hard to debug.

The CRT also manages the boundary between your program and the operating system. On Linux, glibc provides dynamic linking, thread-local storage setup, and the dynamic linker (`ld.so`). On Windows, MSVC offers a choice between dynamic and static CRT linking, with the static option creating heap isolation traps between DLLs. These platform differences are not just implementation details — they affect binary size, startup time, memory safety, and deployment.

The key mental model is that the CRT is **not optional** — even freestanding environments must provide a minimal startup sequence. The difference between hosted and freestanding environments is directly how much of the CRT you get: hosted gives you everything (heap, exceptions, standard library), while freestanding gives you almost nothing and requires you to provide your own `_start` and manage the stack manually. Knowing where the CRT ends and your code begins is essential for systems programming, embedded development, and understanding what happens at program startup.