...
<aside aria-label="`std::terminate` vs `std::exit` If an exception escapes `main`Or an unjoinable" class="starlight-aside starlight-aside--caution">
...
<aside aria-label="Dynamic TLS has a significant first-access penalty (guard variable check, potential" class="starlight-aside starlight-aside--caution">
...
<aside aria-label="`LD_PRELOAD` interposes symbols but does not change `.init_array` ordering. A preloaded" class="starlight-aside starlight-aside--caution">
...
<aside aria-label="The heap is **not** thread-safe at initialization time. If a global constructor spawns a" class="starlight-aside starlight-aside--caution">
...
<aside aria-label="Per [N4950 S6.6.1], the `main` function signature with `char* envp[]` as a third parameter" class="starlight-aside starlight-aside--note">
