...
<Aside type="caution" title="`std::terminate` vs `std::exit` If an exception escapes `main`Or an unjoinable">
...
<Aside type="caution" title="Dynamic TLS has a significant first-access penalty (guard variable check, potential">
...
<Aside type="caution" title="`LD_PRELOAD` interposes symbols but does not change `.init_array` ordering. A preloaded">
...
<Aside type="caution" title="The heap is **not** thread-safe at initialization time. If a global constructor spawns a">
...
<Aside type="note" title="Per [N4950 S6.6.1], the `main` function signature with `char* envp[]` as a third parameter">
