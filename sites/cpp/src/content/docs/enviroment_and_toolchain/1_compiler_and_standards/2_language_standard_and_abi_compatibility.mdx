...
<Aside type="caution" title="Library Availability On Linux, using `-stdlib=libc++` requires the installation of">
...
<Aside type="caution" title="Mixing Clang and GCC on Linux with the same `libstdc++` is generally safe for the same">
