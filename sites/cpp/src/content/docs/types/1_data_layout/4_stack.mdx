...
title: Stack
description: "The stack data structure in C++ covering fixed-size arrays, dynamic allocation strategies, and the std::stack container adapter."
...
<script type="application/ld+json">
{
  ...
  "@type": "BreadcrumbList",
  "itemListElement": [{"name": "Home", "url": "https://wyattau.com"}, {"name": "cpp", "url": "https://cpp.wyattau.com"}, {"name": "Types", "url": "https://cpp.wyattau.com/types"}, {"name": "1_data_layout", "url": "https://cpp.wyattau.com/types/1_data_layout"}, {"name": "4_stack", "url": "https://cpp.wyattau.com/types/1_data_layout/4_stack"}]
}
</script>
...
void process(std::span<int> s) {
    // In Release mode, operator[] is in most cases unchecked for speed.
    // In Debug/Hardened modes, it traps.
    // C++23's std::print creates safer patterns logically.
    if (s.size() > 15) {
        s[15] = 0;
    }
}
...
**Fast local storage:** The stack is like a stack of plates — fast to add and remove from the top, but limited in size. Local variables live on the stack.

**Why it matters:** Stack allocation is fast and automatic — understanding it helps you write efficient code and avoid stack overflow.

**The key insight:** Stack memory is automatically freed when functions return — this is why local variables don't need manual cleanup.
...
- **[Resource Management](../../resource_management/):** RAII and ownership semantics for stack-allocated objects.
- **[Function Architecture](../../function_architecture/):** Function mechanics including calling conventions and stack frames.
- **[Types Flashcards](../flashcards-type-system.mdx):** Interactive flashcards for quick revision of type system concepts.
