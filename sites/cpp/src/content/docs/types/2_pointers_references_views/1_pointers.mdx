...
- [The Stack](/types/1_data_layout/4_stack/)
- [The Heap](/types/1_data_layout/5_heap/)
- [Reference Lifetime](/types/2_pointers_references_views/2_reference_lifetime/)
- [Type Punning](/types/2_pointers_references_views/3_type_punning/)

```mermaid
flowchart TD
    A[Pointers] --> B[Raw Pointers]
    A --> C[Smart Pointers]
    A --> D[References]
    B --> E[T* ptr]
    B --> F[nullptr]
    ...
    D --> L[T& ref]
    D --> M[T&& rvalue ref]
    I --> O[Exclusive ownership]
    I --> P[Move-only]
    J --> R[Shared ownership]
    K --> U[Observes shared_ptr]
    L --> W[Must be initialised]
    H --> Y[Manual memory management]
```
