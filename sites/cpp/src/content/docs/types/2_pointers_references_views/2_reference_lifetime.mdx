...
description: "A C++ Reference () is an alias for an existing object. Unlike pointers, references are not Objects themselves in the abstract machine; they imply a direct"
...
**Safe references:** Reference lifetime rules ensure references don't outlive the objects they point to — like a library card that expires when you leave the library.

**Why it matters:** Understanding reference lifetime prevents dangling references and undefined behavior.

**The key insight:** Temporary objects are destroyed at the end of the full expression — this is when their references become dangling.
