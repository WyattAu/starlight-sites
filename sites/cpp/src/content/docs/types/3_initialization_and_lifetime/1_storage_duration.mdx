...
See [Reference Lifetime](/types/2_pointers_references_views/2_reference_lifetime/) for details on
...
| `operator new`            | Array allocation              | Replaceable globally |
...
```mermaid
flowchart TD
    A[1_Storage_Durationx] --> B[Key Concepts]
    A --> C[Core Principles]
