---

date: 2026-07-23T21:57:32+01:00
title: "Diagnostic Test: Climate | DSE"
description: "Comprehensive study notes for Diagnostic Test: Climate with worked examples, practice problems, and key concepts for exam preparation."
sidebar_position: 11
---

<!-- Breadcrumb Schema for SEO -->
<script type="application/ld+json">
{
  "@context": "https://schema.org",
  "@type": "BreadcrumbList",
  "itemListElement": [{"name": "Home", "url": "https://wyattau.com"}, {"name": "dse", "url": "https://dse.wyattau.com"}, {"name": "Geography", "url": "https://dse.wyattau.com/geography"}, {"name": "Diagnostics", "url": "https://dse.wyattau.com/geography/diagnostics"}, {"name": "Diag Climate", "url": "https://dse.wyattau.com/geography/diagnostics/diag-climate"}]
}
</script>


```mermaid
flowchart TD
    A[Diag Climate] --> B[Key Concepts]
    A --> C[Core Principles]
    A --> D[Practical Applications]
    B --> E[Fundamental definitions]
    C --> F[Design patterns]
    D --> G[Real-world usage]
```

## Diagnostic Test: Climate

Answer each question. Check your answers against the key at the end.

## Questions

**1.** Which pressure belt is primarily responsible for the formation of trade winds?
(A) Polar high
(B) Subtropical high
(C) Equatorial low
(D) Subpolar low

**2.** The East Asian monsoon in Hong Kong brings wet summers due to:
(A) Cold, dry air flowing from Siberia
(B) Warm, moist maritime air from the Pacific Ocean
(C) Descending air from the subtropical ridge
(D) Polar easterlies crossing the equator

**3.** Which category on the Saffir-Simpson scale describes a typhoon with sustained winds of 180 km/h?
(A) Tropical storm
(B) Category 1
(C) Category 3
(D) Category 5

**4.** What is the primary mechanism by which the Intergovernmental Panel on Climate Change (IPCC) attributes global warming to human activity?
(A) Analysis of tree rings alone
(B) Detection of greenhouse gas increases and matching observed warming patterns
(C) Measurement of ocean salinity changes
(D) Satellite monitoring of deforestation rates

**5.** Which atmospheric circulation cell extends from approximately 30 degrees to 60 degrees latitude?
(A) Hadley cell
(B) Ferrel cell
(C) Polar cell
(D) Walker cell

**6.** Hong Kong's average annual rainfall is approximately:
(A) 500 mm
(B) 1,200 mm
(C) 2,400 mm
(D) 3,600 mm

**7.** The greenhouse effect is enhanced primarily by increased atmospheric concentrations of:
(A) Oxygen and nitrogen
(B) Carbon dioxide, methane, and nitrous oxide
(C) Argon and helium
(D) Water vapour exclusively

**8.** Which of the following is a direct consequence of tropical cyclones affecting Hong Kong?
(A) Prolonged drought conditions
(B) Heavy rainfall, storm surge, and potential flooding
(C) Persistent frost and freezing temperatures
(D) Increased earthquake activity

**9.** The IPCC's Representative Concentration Pathways (RCPs) represent:
(A) Historical temperature records from weather stations
(B) Different scenarios of future greenhouse gas concentration trajectories
(C) Regional climate variations across continents
(D) Ocean current circulation patterns

**10.** Which factor most significantly influences seasonal temperature variation in Hong Kong?
(A) Distance from the equator and monsoon influence
(B) Altitude above sea level
(C) Proximity to the Arctic Circle
(D) Annual volcanic activity

## Intuition

**Earth's thermostat:** Climate is like the planet's long-term mood — it's the average weather pattern over decades, not the day-to-day fluctuations. The atmosphere is a giant heat engine powered by the sun.

**Why it matters:** From agriculture to urban planning, climate determines where people live, what they grow, and how they build. Understanding climate systems helps predict droughts, floods, and typhoons.

**The key insight:** The greenhouse effect isn't inherently bad — without it, Earth would be frozen. The problem is the enhanced greenhouse effect from human emissions.

## Answer Key

| Q | Answer | Explanation |
| --- | -------- | ------------- |
| 1 | (B) | The subtropical high-pressure belt around 30 degrees latitude drives trade winds toward the equator as part of the Hadley cell circulation. |
| 2 | (B) | The summer monsoon brings warm, moist air from the Pacific, producing Hong Kong's wet season from May to September. |
| 3 | (C) | Category 3 hurricanes (typhoons) on the Saffir-Simpson scale have sustained winds of 178-208 km/h, causing extensive damage. |
| 4 | (B) | The IPCC uses detection and attribution studies comparing observed warming with model predictions driven by greenhouse gas increases. |
| 5 | (B) | The Ferrel cell operates between 30 and 60 degrees latitude, driven by surface friction and the Hadley and Polar cells. |
| 6 | (C) | Hong Kong receives approximately 2,400 mm of rainfall annually, concentrated in the wet season. |
| 7 | (B) | Carbon dioxide, methane, and nitrous oxide are the principal anthropogenic greenhouse gases enhancing the natural greenhouse effect. |
| 8 | (B) | Tropical cyclones bring intense rainfall, storm surge, and flooding, posing significant hazards to Hong Kong. |
| 9 | (B) | RCPs are scenarios used in IPCC reports projecting future greenhouse gas concentrations and their climate impacts. |
| 10 | (A) | Hong Kong's seasonal temperatures are primarily influenced by latitude and the alternation between summer and winter monsoons. |

## Common Mistakes

**Confusing climate zones with latitude alone:** Climate is affected by latitude, altitude, ocean currents, and continental position. Two places at the same latitude can have very different climates.

**Assuming all tropical cyclones are called hurricanes:** They're called typhoons in the western Pacific, cyclones in the Indian Ocean, and hurricanes in the Atlantic. Same phenomenon, different regional names.

**Mixing up greenhouse effect with ozone depletion:** The greenhouse effect traps heat using CO₂ and other gases. Ozone depletion involves CFCs destroying the ozone layer. They're separate environmental issues.

## Cross-References

- **[Population](../population):** Population dynamics are studied
- **[Urbanization](../urbanization):** Urbanisation is a major topic
- **[Agriculture](../agriculture):** Agriculture is linked to population
