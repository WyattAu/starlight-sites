...
title: "Functions | DSE - Wyatt's Notes"
...
Functions are the fundamental building blocks of mathematics, mapping inputs to outputs like a machine that transforms raw materials into products. The domain is what you feed in, the codomain is what you could get, and the range is what you actually get. Composite functions chain machines together, while inverse functions reverse the process. Transformations shift, stretch, and flip graphs, providing a geometric interpretation of algebraic changes. Understanding functions is essential because they model relationships between quantities in every area of mathematics and science.
...
<DesmosGraph client:only="solid-js" title="Graph Transformations" expressions={["a\cdot f(x+b)+c", "f(x)=x^2"]} width={800}
height={500}  />
...
1. **Question:** Let $f(x) = 3x - 5$. Find $f^{-1}(x)$ and verify that $f^{-1}(f(x)) = x$.
...
1. **Question:** A quadratic function $f(x)$ has a maximum value of $7$ at $x = 2$ And $f(0) = -5$.
   Find $f(x)$.
...
1. **Question:** Find the range of values of $k$ for which the equation
...
1. **Question:** Solve the equation $3^{2x} - 10 \cdot 3^x + 9 = 0$.
...
1. **Question:** Solve $\log_3(x - 2) + \log_3(x + 6) = 2$.
...
1. **Question:** The graph of $y = f(x)$ passes through the points $(1, 4)$ and $(3, 10)$. State the
...
1. **Question:** Given $f(x) = \log_2(x + 3)$ and $g(x) = 2^x - 1$Show that $f$ and $g$ are Inverse
...
1. **Question:** Given $f(x) = x^2 + 2x - 3$Find the range of $f$ when (a) the domain is
...
1. **Question:** Let $f(x) = \dfrac{x}{x - 2}$ for $x \neq 2$. Find $f^{-1}$ And evaluate
    $f^{-1}(3) + f(3)$.
...
- [Calculus](../../../../../../ib/src/content/docs/maths/5-calculus/2_differentiation) applies differentiation to analyse rates of change in the functions defined here.
- [Sequences and Series](6_sequences-and-series) extends function concepts to infinite sequences and series used in mathematical modelling.
- [Trigonometry](../../../../../../alevel/src/content/docs/maths/pure-mathematics/05-functions) covers the periodic functions that are a special case of the general function framework.
