...
Quadratic equations describe parabolic relationships that appear throughout physics and engineering, from projectile motion to revenue optimisation. The quadratic formula is the universal solver, derived by completing the square on the general form. The discriminant acts as a diagnostic tool, telling you without solving whether the parabola crosses the x-axis twice, touches once, or misses entirely. Completing the square transforms any quadratic into vertex form, revealing the turning point directly. The symmetry of parabolas means the axis of symmetry passes through the vertex, equidistant from both roots.
...
```mermaid
flowchart TD
    A[3_Quadraticsx] --> B[Key Concepts]
    A --> C[Core Principles]
...
1. **Question:** Find the values of $k$ for which $kx^2 + 2x + k = 0$ has two distinct real roots.

$\Delta = 4 - 4k^2 \gt 0 \implies k^2 \lt 1 \implies -1 \lt k \lt 1$ (and $k \neq 0$ since it must
Be quadratic).

1. **Question:** Express $f(x) = -x^2 + 6x - 5$ in completed square form and state its maximum
   value.

$f(x) = -(x^2 - 6x) - 5 = -[(x - 3)^2 - 9] - 5 = -(x - 3)^2 + 4$. Maximum value: $4$ at $x = 3$.

1. **Question:** Solve the inequality $2x^2 + 3x - 5 \lt 0$.

$2x^2 + 3x - 5 = (2x + 5)(x - 1)$. Roots: $x = -5/2$ and $x = 1$. Parabola opens upward. Solution:
$-5/2 \lt x \lt 1$.

1. **Question:** If $\alpha$ and $\beta$ are roots of $x^2 - 3x + 5 = 0$Form a quadratic equation
...
1. **Question:** A ball is thrown upward. Its height $h$ metres at time $t$ seconds is
...
1. **Question:** Find the range of the function $f(x) = 2x^2 - 8x + 11$.

$f(x) = 2(x^2 - 4x) + 11 = 2[(x - 2)^2 - 4] + 11 = 2(x - 2)^2 + 3$. Since $2(x - 2)^2 \geqslant 0$
The minimum is $3$. Range: $[3, \infty)$.

1. **Question:** The equation $x^2 + 2px + p + 8 = 0$ has real roots. Find the range of $p$.
...
1. **Question:** Prove that the quadratic equation $(a^2 + b^2)x^2 + 2(ac + bd)x + (c^2 + d^2) = 0$
...
1. **Question:** For what integer values of $k$ does the equation $x^2 - kx + k + 3 = 0$ have
    integer roots?
...
- **Functions:** Quadratic functions are a special case of polynomial functions. See
  [Functions](/maths/compulsory/1_functions/) and [Functions Advanced](/maths/compulsory/2_functions-advanced/).
...
- **Permutations and Combinations:** Factorials appear in the quadratic formula derivation via
  completing the square. See [Permutations and Combinations](/maths/compulsory/13_permutations-and-combinations/).
...
<Aside type="tip">
Diagnostic Test Ready to test your understanding of **Quadratics**? The contains the hardest questions within
