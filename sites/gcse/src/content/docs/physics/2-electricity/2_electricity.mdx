...
**Electricity is the flow of charge — and it powers everything:** Think of electricity as water flowing through pipes. The battery is the pump, the wires are the pipes, and the components (lamps, motors) are the taps that use the flow. Voltage is the pressure pushing the charges through, current is how much charge flows per second, and resistance is how much the pipes restrict the flow.

**Why it matters:** Electricity powers every modern device — from your phone to traffic lights to hospitals. Understanding circuits explains why fuses blow, whyLEDs are efficient, and how solar panels generate power. It's the foundation of all electronics.

**The key insight:** In a series circuit, current is the same everywhere but voltage splits across components. In a parallel circuit, voltage is the same across branches but current splits. This symmetry is the key to solving any circuit problem.
...
1. A circuit contains a $9 \mathrm{ V$ battery, a $100 \mathrm{ \Omega$ resistor in series with a
...
2. A student connects an ammeter in parallel with a resistor. Explain why this is dangerous and
    what will happen.

3. A $6 \mathrm{ V$ battery with internal resistance $0.3 \mathrm{ \Omega$ is connected to an
    external circuit. The terminal PD is $5.4 \mathrm{ V$. Calculate the current and the external
    resistance.

4. Design a potential divider circuit using a $9 \mathrm{ V$ battery and two resistors that
    produces an output voltage of $3 \mathrm{ V$. Specify the values of both resistors.

5. Explain the difference between a thermistor and an LDR. Describe a sensing circuit for each and
    explain how the output voltage changes as the sensed quantity changes.

6. A hairdryer rated at $1800 \mathrm{ W$ is connected to the $230 \mathrm{ V$ mains. Calculate the
...
7. Three identical resistors, each of resistance $R$Are connected to a battery. Calculate the total
    resistance when they are connected (a) all in series, (b) all in parallel, and (c) two in
    parallel with the third in series.

8. Explain why the resistance of a filament lamp increases with temperature, while the resistance
    of a thermistor (NTC) decreases with temperature.

9. A mobile phone charger has an output of $5 \mathrm{ V$ and $2 \mathrm{ A$. If the phone battery
...
10. A circuit contains a variable resistor, a fixed resistor of $470 \mathrm{ \Omega$ And an LED in
...
:::note
<strong>Info Heating appliances dominate household electricity bills. Switching to a more efficient</strong>
...
:::caution
<strong>Warning The energy is dissipated as heat inside the battery. Never short-circuit a</strong>
...
1. A $12 \mathrm{ V$ car battery has internal resistance $0.05 \Omega$. The starter motor draws
    $200 \mathrm{ A$. Calculate (a) the terminal PD, (b) the power delivered to the starter, and (c)
    the power wasted in the battery.

2. Three identical resistors each of $R$ are connected to a battery. Calculate the total resistance
    for all possible arrangements and rank them from largest to smallest.

3. An electric shower has a power rating of $8.5 \mathrm{ kW$ and operates at $230 \mathrm{ V$.
    Calculate the current it draws and explain why it requires a dedicated high-current circuit.

4. A student sets up a circuit with a thermistor and a fixed resistor in series with a
...
5. Explain the difference between conventional current and electron flow. Why do we still use
...
```mermaid
flowchart TD
    A[2_Electricityx] --> B[Key Concepts]
    A --> C[Core Principles]
...
- [Energy](../1-energy/1_energy) -- Electrical circuits involve energy transfers; power and energy are calculated using VIt and I²Rt.
- [Magnetism](../7-magnetism/7_magnetism) -- Electromagnetism connects electric currents to magnetic fields in motors and generators.
- [Waves](../6-waves/6_waves) -- Alternating current produces electromagnetic waves; understanding frequency relates to AC circuits.
- [Forces](../5-forces/5_forces) -- Electric motors convert electrical energy into mechanical force, linking electricity to motion.
