...
description: "Physical quantities are either (magnitude only) or (magnitude and Direction). Comprehensive educational content coverage with definitions and practice problems."
date: 2026-04-14
tags:
  - highers
  - highers-physics
...
1. A stone is thrown at $18 \mathrm{ m/s$ at $50^\circ$ to the horizontal from the top of a
    $25 \mathrm{ m$ cliff. Calculate the speed at which it hits the ground.

2. A car of mass $900 \mathrm{ kg$ travels at constant speed around a circular bend of radius
    $60 \mathrm{ m$ that is banked at $12^\circ$. Calculate the minimum coefficient of static
    friction required.

3. A bullet of mass $8 \mathrm{ g$ travelling at $600 \mathrm{ m/s$ passes through a wooden block
    of mass $2 \mathrm{ kg$Emerging at $200 \mathrm{ m/s$. Calculate the velocity of the block and
    the kinetic energy lost.

4. A $5 \mathrm{ kg$ box is pushed up a $35^\circ$ incline at constant speed by a force of
    $50 \mathrm{ N$ applied parallel to the incline. Calculate the coefficient of kinetic friction.

5. A ball of mass $0.3 \mathrm{ kg$ is attached to a string of length $0.8 \mathrm{ m$ and swung in
    a vertical circle. Calculate the minimum speed at the lowest point for the ball to complete the
    full circle.

6. Two objects of masses $3 \mathrm{ kg$ and $5 \mathrm{ kg$Connected by a light string, are placed
...
7. A satellite of mass $500 \mathrm{ kg$ orbits at a height of $600 \mathrm{ km$ above Earth\'s
    surface. Calculate the orbital speed, period, and gravitational potential energy of the
    satellite.

8. A $2 \mathrm{ kg$ object slides from rest down a frictionless curved ramp of total height
    $4 \mathrm{ m$. At the bottom, it enters a rough horizontal surface with $\mu_k = 0.3$. How far
    does it slide before stopping?

9. A crane lifts a $300 \mathrm{ kg$ load with an upward acceleration of $1.5 \mathrm{ m/s^2$ for
...
10. Explain why the range of a projectile is the same for complementary angles (e.g., $30^\circ$ and
...
:::note
<strong>The work-energy theorem is one of the most powerful tools in mechanics. It works even when</strong>
...
:::caution
<strong>Always check whether motion actually occurs before applying the equations of motion. If</strong>
...
1. A $0.5 \mathrm{ kg$ ball is thrown horizontally from a cliff $40 \mathrm{ m$ high at
...
2. A car of mass $1200 \mathrm{ kg$ travels at $20 \mathrm{ m/s$ on a level road. The engine
...
3. A $60 \mathrm{ kg$ person stands on a spring scale in a lift. Calculate the scale reading when
...
4. Two particles of masses $0.3 \mathrm{ kg$ and $0.5 \mathrm{ kg$ approach each other with speeds
...
5. Derive an expression for the period of a simple pendulum using energy considerations. A pendulum
...
- [Dynamics and Space](../2-dynamics-space/2_dynamics-space) -- Forces cause acceleration, connecting the kinematic description of motion to the dynamic explanation.
- [Waves and Radiation](../5-waves-radiation/5_waves-radiation) -- Simple harmonic motion in mechanics shares mathematical treatment with wave phenomena.
- [Algebra and Functions](../../maths/1-algebra-functions/1_algebra-functions) -- Kinematic equations are algebraic relationships between displacement, velocity, and time.
...
Mechanics is the study of **why things move the way they do**. At its heart, it rests on Newton's three laws, which connect force, mass, and acceleration into a single framework. The first law tells you that motion persists without force; the second tells you that force changes motion; the third tells you that forces always come in pairs. Everything else in mechanics — friction, energy, momentum, circular motion — is an application of these three principles to specific situations. The key insight is that force is a vector: you must resolve it into components before applying Newton's second law.

Energy and momentum are **conservation laws** — quantities that remain constant unless an external influence changes them. Energy conservation is particularly powerful because it lets you solve problems without tracking the detailed path of motion. If you know the initial and final speeds and the forces involved, you can skip the acceleration and time calculations entirely. Momentum conservation is the tool for collision problems: when two objects interact, the total momentum before equals the total, regardless of what happens during the collision.

The intuition for circular motion is that **something must constantly pull the object inward** to keep it moving in a circle. Without this centripetal force, the object flies off in a straight line (Newton's first law). The centripetal force is not a new type of force — it is the net inward component of whatever forces are already present: tension in a string, friction on a road, gravity in an orbit. Understanding this distinction is the key to correctly solving circular motion problems.
