...
1. A copper wire of length $20 \mathrm{ m$ and diameter $1.0 \mathrm{ mm$ has a resistance of
    $0.35 \Omega$. Calculate the resistivity of copper.

2. In a circuit, a $24 \mathrm{ V$ battery with internal resistance $0.6 \Omega$ is connected to
...
3. A $470 \mu\mathrm{F$ capacitor is charged to $25 \mathrm{ V$ and then discharged through a
...
4. A potential divider consists of a $15 \mathrm{ k\Omega$ resistor and an LDR in series with a
...
5. Two capacitors of $47 \mu\mathrm{F$ and $100 \mu\mathrm{F$ are connected in series and charged
...
6. Explain why the total capacitance of capacitors in parallel is greater than any individual
    capacitance, whereas the total resistance of resistors in parallel is less than any individual
    resistance.

7. A battery of unknown EMF is connected to a $6 \Omega$ resistor and the current is
...
8. A $100 \mu\mathrm{F$ capacitor charged to $50 \mathrm{ V$ is connected across an uncharged
    $47 \mu\mathrm{F$ capacitor. Calculate the final voltage across both capacitors and the energy
    lost in the process.

9. Design a potential divider circuit using a $12 \mathrm{ V$ supply that produces an output of
    $4 \mathrm{ V$ when a thermistor has resistance $8 \mathrm{ k\Omega$. Specify the value of the
    fixed resistor.

10. Explain how a capacitor can be used to smooth the output of a half-wave rectifier. Include a
...
1. A $470 \mu\mathrm{F$ capacitor is charged to $20 \mathrm{ V$ and then discharged through a
...
2. Three resistors of $10 \Omega$, $20 \Omega$ And $30 \Omega$ are connected to a $24 \mathrm{ V$
...
3. An AC circuit has $R = 80 \Omega$, $X_L = 100 \Omega$ And $X_C = 40 \Omega$ in series with a
    $230 \mathrm{ V$, $50 \mathrm{ Hz$ supply. Calculate the impedance, current, power factor, and
    true power dissipated.

4. Design a potential divider using a $9 \mathrm{ V$ battery and two resistors to produce an output
...
5. Explain why domestic appliances are connected in parallel rather than series. Include
...
1. A battery with EMF $15 \mathrm{ V$ and internal resistance $0.8 \Omega$ is connected to an
...
2. A $100 \mu\mathrm{F$ capacitor is charged to $30 \mathrm{ V$ and then connected across a
    $200 \mu\mathrm{F$ uncharged capacitor. Calculate the final voltage across both, the charge on
    each, and the energy lost.

3. An AC circuit has a $60 \Omega$ resistor and a $0.1 \mathrm{ H$ inductor in series with a
...
4. A potential divider circuit uses a $10 \mathrm{ k\Omega$ variable resistor and an LDR. In
...
5. Explain the difference between EMF and potential difference. Why can a voltmeter give a reading
    less than the EMF when connected across a battery supplying current to a circuit?

## Cross-References

- [Mechanics](../1-mechanics/1_mechanics) -- Electric circuits involve energy transfer, connecting electrical power to mechanical work and energy conservation.
- [Waves and Radiation](../5-waves-radiation/5_waves-radiation) -- Electromagnetic waves are produced by accelerating charges, linking electricity to wave phenomena.
- [Dynamics and Space](../2-dynamics-space/2_dynamics-space) -- Electric and magnetic fields affect the motion of charged particles, connecting electricity to dynamics.
...
Electricity is about the flow of **charge** — and the central relationship is Ohm's law, which connects voltage (the push), current (the flow), and resistance (the opposition). Think of voltage as water pressure, current as water flow rate, and resistance as pipe narrowness. This analogy extends to circuits: resistors in series add like pipes in a chain, resistors in parallel add like parallel pipes (reducing total resistance), and power dissipation is the "heat" generated by pushing current through resistance.

Capacitors store energy in electric fields, and their behaviour in circuits is fundamentally different from resistors. A capacitor resists *changes* in voltage — it charges exponentially when connected to a supply and discharges exponentially when the supply is removed. The time constant $\tau = RC$ tells you how quickly this happens: after one time constant, a capacitor is 63.2% charged; after five time constants, it is effectively fully charged. This exponential behaviour is the mathematical signature of any system where the rate of change is proportional to the current state.

Electromagnetic induction is the reverse of how a motor works: changing magnetic flux through a coil induces an EMF. Faraday's law tells you the magnitude ($\varepsilon = -d\Phi/dt$), and Lenz's law tells you the direction (the induced current opposes the change in flux). Transformers exploit this principle to step voltage up or down for efficient power transmission — stepping up to high voltage for long-distance transmission (reducing $I^2R$ losses), then stepping down to safe levels for domestic use.
