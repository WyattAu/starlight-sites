...
**The key insight:** ATP is the universal energy currency of cells, connecting energy-releasing reactions to energy-requiring ones
...
1. **Glyceraldehyde-3-phosphate dehydrogenase** oxidises G3P to 1,3-bisphosphoglycerate (1,3BPG),
   reducing $\mathrm{NAD}^+$ to NADH and incorporating inorganic phosphate ($\mathrm{P_i}$).
2. **Phosphoglycerate kinase** transfers a phosphate from 1,3BPG to ADP, producing
   3-phosphoglycerate (3PG) and 1 ATP (**substrate-level phosphorylation**). Per glucose: $2$ ATP.
3. **Phosphoglycerate mutase** converts 3PG to 2-phosphoglycerate (2PG).
4. **Enolase** dehydrates 2PG to phosphoenolpyruvate (PEP).
5. **Pyruvate kinase** transfers a phosphate from PEP to ADP, producing pyruvate and 1 ATP. Per
...
1. Together, these form the **proton-motive force (PMF)**:
   $$\Delta p = \Delta\Psi - \frac{2.303 RT}{F} \Delta\mathrm{pH}$$
2. Protons flow back into the matrix through **ATP synthase** (Complex V), driving the
...
1. Light energy excites an electron in **P700** (reaction centre of PSI).
2. The excited electron passes to ferredoxin (Fd) and then to **NADP$^+$ reductase**, which reduces
...
1. **Glyceraldehyde-3-phosphate dehydrogenase**: G3P + $\mathrm{NAD}^+$ + $\mathrm{P_i}$ $\to$
...
2. **Phosphoglycerate kinase**: 1,3BPG + ADP $\to$ 3-phosphoglycerate (3PG) + ATP. **Substrate-level
   phosphorylation.** Since each glucose produces 2 G3P, this step produces 2 ATP (recovery of the 2
   ATP invested).

3. **Phosphoglycerate mutase**: 3PG $\to$ 2-phosphoglycerate (2PG).

4. **Enolase**: 2PG $\to$ phosphoenolpyruvate (PEP) + $\mathrm{H}_2\mathrm{O}$. Elimination of water
   creates a high-energy phosphate bond.

5. **Pyruvate kinase**: PEP + ADP $\to$ pyruvate + ATP. **Substrate-level phosphorylation.**
