...
<script type="application/ld+json">
{
  ...
  "@type": "BreadcrumbList",
  "itemListElement": [{"name": "Home", "url": "https://wyattau.com"}, {"name": "ib", "url": "https://ib.wyattau.com"}, {"name": "Physics", "url": "https://ib.wyattau.com/physics"}, {"name": "4 Fields", "url": "https://ib.wyattau.com/physics/4-fields"}, {"name": "2_electric And Magnetic Fields", "url": "https://ib.wyattau.com/physics/4-fields/2_electric-and-magnetic-fields"}]
}
</script>
...
<strong>Coulomb\'s Law can also be expressed in vector form, where the force exerted by $q_1$ on</strong>
...
<img src="/img/physics/CoulombsLaw.png" alt="Coulombs_law" width="416" height="101" loading="lazy" />
</aside>
...
<aside class="starlight-aside starlight-aside--note">
<strong>The derivation of Coulomb\'s constant can be possible with Maxwell\'s equations. Gauss\'s Law</strong>
...
Electric and magnetic fields are invisible landscapes that exert forces on charges. Electric field lines map out the terrain a positive test charge would feel, flowing outward from positive sources and inward toward negative ones. Coulomb's law is an inverse-square law like gravity, but charges can both attract and repel. Capacitors store energy in the electric field between their plates like water behind a dam. Magnetic fields arise from moving charges and always form closed loops, unlike electric field lines that start and end on charges. The key difference is that magnetic forces are always perpendicular to motion, so they change direction but never speed.
...
<strong>Diagnostic Test Ready to test your understanding of **Electric and Magnetic Fields**? The contains the hardest</strong>
...
**Confusing electric field strength with electric potential:** Field strength ($E$) is a vector (force per unit charge); potential ($V$) is a scalar (energy per unit charge). Field lines show direction of force, not potential. Students often conflate the two, leading to incorrect force calculations.

**Forgetting that magnetic force is always perpendicular to velocity:** The Lorentz force $\mathbf{F} = q\mathbf{v} \times \mathbf{B}$ is always perpendicular to both velocity and magnetic field. This means magnetic fields do no work — they change direction but not speed. Students often incorrectly calculate work done by magnetic fields.

**Misapplying the right-hand rule:** For positive charges, use the right-hand rule: fingers point in direction of $\mathbf{v}$, curl toward $\mathbf{B}$, thumb points to force. For negative charges, reverse the direction. Students frequently forget to reverse for electrons.

**Confusing series and parallel capacitor combinations:** Capacitors in series combine like resistors in parallel: $\frac{1}{C_{total}} = \frac{1}{C_1} + \frac{1}{C_2}$. Capacitors in parallel combine like resistors in series: $C_{total} = C_1 + C_2$. This is the opposite of resistors — a common source of errors.

**Forgetting that induced EMF opposes the change:** Lenz's law states that induced current flows in a direction that opposes the change in flux. Students often get the direction wrong by forgetting this fundamental principle.
