...
- [Types and Variables](/languages/python/02-fundamentals/01-types-and-variables.md) — Boolean truthiness, None checks, and the __bool__ protocol determine how conditions evaluate in control flow.
- [Collections](/languages/python/03-data-structures/01-collections.md) — Iterating over lists, dicts, and sets in for loops uses the iterator protocol that underpins all collection types.
- [Python Internals](/languages/python/02-fundamentals/05-python-internals.md) — The bytecode instructions generated by if/else and for loops are visible in the dis module output.
- [Dicts, Sets, and Collections Deep Dive](/languages/python/03-data-structures/02-dicts-sets-counter.md) — Pattern matching and dictionary dispatch relate to the dict-based dispatch patterns in collections.
