---
title: "Complete Irish Leaving Certificate Study Guide"
description: "Comprehensive Irish Leaving Certificate study guide covering Biology, Chemistry, Computer Science, Mathematics, and Physics. Topic notes, practice questions, flashcards, and diagnostics for the Irish state examinations."
date: 2026-07-24
tags:
  - leaving-cert
  - study-guide
  - irish
  - exam-preparation
categories:
  - hub
---

<script type="application/ld+json">
{
  "@context": "https://schema.org",
  "@type": "BreadcrumbList",
  "itemListElement": [
    {"name": "Home", "url": "https://leaving-cert.wyattau.com"},
    {"name": "Hub", "url": "https://leaving-cert.wyattau.com/hub"}
  ]
}
</script>

<script type="application/ld+json">
{
  "@context": "https://schema.org",
  "@type": "Course",
  "name": "Complete Irish Leaving Certificate Study Guide",
  "description": "Comprehensive Leaving Certificate study guide covering Biology, Chemistry, Computer Science, Mathematics, and Physics with topic notes, flashcards, practice questions, and diagnostics.",
  "provider": {
    "@type": "Organization",
    "name": "Wyatt's Notes",
    "url": "https://leaving-cert.wyattau.com"
  },
  "url": "https://leaving-cert.wyattau.com/hub",
  "educationalLevel": "Secondary",
  "inLanguage": "en",
  "isAccessibleForFree": true
}
</script>

## Why This Guide Exists

The Leaving Certificate is the state examination taken by students in Ireland at the end of secondary school, typically at age 17–18. It is the primary qualification used for university admissions in Ireland and is recognised by universities across the UK and internationally. Students typically sit 6–7 subjects, with the best 6 counting for their CAO points score.

This hub page links every resource on this site — organised by subject, with study plans and exam strategies tailored to the Irish exam system. Every topic below links to detailed notes, practice questions, flashcards, and diagnostic quizzes.

## Table of Contents

- [Biology](#biology)
- [Chemistry](#chemistry)
- [Computer Science](#computer-science)
- [Mathematics](#mathematics)
- [Physics](#physics)
- [Recommended Study Plan](#recommended-study-plan)
- [Cross-Site Resources](#cross-site-resources)
- [FAQ](#faq)

---

## Biology

Leaving Cert Biology covers cell biology, ecology, genetics, plant biology, and the human body. The exam tests both factual knowledge and the ability to apply biological understanding to practical scenarios.

### Topic Notes

- [Biology Overview](biology) — course structure and assessment format
- [Cell Biology](biology/1-cell) — cell structure, organelles, and cell division
- [Ecology](biology/2-ecology) — ecosystems, energy flow, and nutrient cycling
- [Genetics](biology/3-genetics) — inheritance, DNA, and gene expression
- [Plant Biology](biology/4-plant-biology) — photosynthesis, transpiration, and plant responses
- [Human Body](biology/5-human-body) — organ systems, homeostasis, and disease

### Practice and Review

- [Flashcards: Biology](flashcards-biology)
- [Practice Questions: Biology](biology/practice-biology)
- [Diagnostic Quizzes](biology/diagnostics) — test across all biology topics

### Key Exam Focus

Leaving Cert Biology emphasises practical skills — drawing and labelling diagrams, interpreting experimental results, and designing investigations. The exam includes structured questions, data analysis, and extended-response questions. Practise drawing biological diagrams accurately with clear labels.

---

## Chemistry

Leaving Cert Chemistry covers atomic structure, bonding, stoichiometry, acids and bases, and organic chemistry. The subject requires fluency with calculations and a strong understanding of chemical principles.

### Topic Notes

- [Chemistry Overview](chemistry) — course structure and assessment format
- [Atomic Structure](chemistry/1-atomic-structure) — electron configuration, periodic trends, and isotopes
- [Bonding](chemistry/2-bonding) — ionic, covalent, and metallic bonding
- [Stoichiometry](chemistry/3-stoichiometry) — mole concept, balancing equations, and yield calculations
- [Acids and Bases](chemistry/4-acids-bases) — pH, titrations, and neutralisation
- [Organic Chemistry](chemistry/5-organic) — hydrocarbons, functional groups, and reactions

### Practice and Review

- [Flashcards: Chemistry](flashcards-chemistry)
- [Practice Questions: Chemistry](chemistry/practice-chemistry)
- [Diagnostic Quizzes](chemistry/diagnostics) — test across all chemistry topics

### Key Exam Focus

Leaving Cert Chemistry places heavy emphasis on calculations — moles, concentrations, enthalpy changes, and equilibrium. The exam rewards clear, step-by-step working. Practise mole calculations and titration problems until the technique is automatic.

---

## Computer Science

Leaving Cert Computer Science covers hardware, programming, databases, networks, and algorithms. The course balances theoretical knowledge with practical programming skills.

### Topic Notes

- [Computer Science Overview](computer-science) — course structure and assessment format
- [Hardware](computer-science/1-hardware) — computer architecture, processors, and memory
- [Programming](computer-science/2-programming) — variables, control structures, and functions
- [Databases](computer-science/3-databases) — relational databases, SQL, and normalisation
- [Networks](computer-science/4-networks) — network types, protocols, and security
- [Algorithms](computer-science/5-algorithms) — searching, sorting, and complexity analysis

### Practice and Review

- [Flashcards: Computer Science](flashcards-computer-science)
- [Practice Questions: Computer Science](computer-science/practice-computer-science)
- [Diagnostic Quizzes](computer-science/diagnostics) — test across all computer science topics

### Key Exam Focus

Leaving Cert Computer Science tests both theory and practical application. The exam includes pseudocode tracing, algorithm design, database schema design, and short-answer theory questions. Practise writing clear pseudocode and drawing ER diagrams for database problems.

---

## Mathematics

Leaving Cert Mathematics covers algebra, calculus, geometry and trigonometry, probability and statistics, and number sequences. The course is available at Ordinary Level and Higher Level.

### Topic Notes

- [Mathematics Overview](maths) — course structure and assessment format
- [Algebra](maths/1-algebra) — equations, inequalities, and polynomial manipulation
- [Calculus](maths/2-calculus) — differentiation, integration, and applications
- [Geometry and Trigonometry](maths/3-geometry-trig) — triangles, circles, coordinate geometry, and trigonometric identities
- [Probability and Statistics](maths/4-probability-statistics) — probability rules, distributions, and data analysis
- [Number Sequences](maths/5-number-sequences) — arithmetic and geometric sequences and series

### Practice and Review

- [Flashcards: Mathematics](flashcards-maths)
- [Practice Questions: Mathematics](maths/practice-maths)
- [Diagnostic Quizzes](maths/diagnostics) — test across all maths topics

### Key Exam Focus

Leaving Cert Mathematics rewards structured, step-by-step working. Show every line of your solution — markers award partial credit for correct method even if the final answer is wrong. Calculus questions are a major component at Higher Level — practise differentiation and integration techniques extensively.

---

## Physics

Leaving Cert Physics covers mechanics, waves, electricity, heat, and modern physics. The subject combines conceptual understanding with mathematical problem-solving.

### Topic Notes

- [Physics Overview](physics) — course structure and assessment format
- [Mechanics](physics/1-mechanics) — motion, forces, and Newton's laws
- [Waves](physics/2-waves) — wave properties, sound, light, and diffraction
- [Electricity](physics/3-electricity) — circuits, voltage, current, resistance, and power
- [Heat](physics/4-heat) — thermal energy, specific heat capacity, and specific latent heat
- [Modern Physics](physics/5-modern-physics) — radioactivity, nuclear reactions, and the photoelectric effect

### Practice and Review

- [Flashcards: Physics](flashcards-physics)
- [Practice Questions: Physics](physics/practice-physics)
- [Diagnostic Quizzes](physics/diagnostics) — test across all physics topics

### Key Exam Focus

Leaving Cert Physics requires you to apply mathematical formulas to physical problems. The exam includes calculation questions, experimental questions, and conceptual explanations. Practise rearranging equations, substituting values, and stating answers with correct units and significant figures.

---

## Recommended Study Plan

Preparing for the Leaving Cert is a marathon. Here is a structured approach that balances all subjects while building deep understanding.

### Phase 1: Foundation (Months 1–4)

- Study all topic notes for each subject, marking areas of uncertainty
- Complete flashcards daily for active recall
- Aim for one diagnostic quiz per subject per week
- Focus on understanding concepts, not memorising facts

### Phase 2: Consolidation (Months 5–8)

- Revisit weak areas identified by diagnostic quizzes
- Begin practice questions under timed conditions
- Work through past Leaving Cert papers — they reveal the exam's style and marking schemes
- Practise extended-response questions for Biology and Science subjects

### Phase 3: Exam Readiness (Months 9–12)

- Focus almost entirely on practice questions and past papers
- Study the marking schemes — they show exactly what markers look for
- Do full-length timed papers under real conditions
- Use flashcards to maintain recall on well-known topics
- Reserve the last two weeks for light revision and rest

### Daily Routine

| Time | Activity |
|------|----------|
| Morning | Active recall — flashcards for 20 minutes |
| Afternoon | Topic study — read notes on one new topic |
| Evening | Practice — complete 5–10 practice questions |
| Weekend | Diagnostic quiz + review weak areas |

---

## Cross-Site Resources

Wyatt's Notes is a network of interconnected study sites. The Leaving Cert content connects to related material:

- **[GCSE Study Guide](https://gcse.wyattau.com/hub)** — comparable UK qualifications if you are comparing exam systems
- **[A-Level Study Guide](https://alevel.wyattau.com/hub)** — the next level after the Leaving Cert
- **[IB Study Guide](https://ib.wyattau.com/hub)** — the International Baccalaureate as an alternative qualification
- **[University Physics](https://physics.wyattau.com/hub)** — deeper coverage beyond Leaving Cert level
- **[University Mathematics](https://mathematics.wyattau.com/hub)** — proof-based mathematics beyond Leaving Cert level

---

## Frequently Asked Questions

### How should I start using this site?

Take a diagnostic quiz for each subject. This takes about 10–15 minutes per subject and gives you a clear picture of your strengths and weaknesses. Then prioritise studying the topics where you scored lowest.

### How many subjects should I study at once?

Most students take 6–7 subjects for the Leaving Cert. Study no more than 3 subjects per day to maintain focus. Alternate between a science, a mathematics, and a humanity each day.

### Are the practice questions similar to real Leaving Cert exams?

The practice questions are modelled on Leaving Cert-style problems. They test the same concepts and use similar formats, but they are not past papers. Use them alongside official SEC past papers and marking schemes for the best preparation.

### How often should I use the flashcards?

Daily. Flashcards use spaced repetition — each time you get a card right, it appears less frequently. This is the most efficient way to retain factual knowledge. Spend 15–20 minutes each morning.

### What CAO points do I need for university?

CAO points vary by course and institution. Generally, competitive courses require 400–600 points. Each Higher Level grade is worth 5–100 points (H1 = 100, H2 = 88, H3 = 77, etc.). Plan your subject choices strategically to maximise your points.

### How do I manage the workload across so many subjects?

Use a study timetable. Allocate specific time blocks to each subject and stick to them. Prioritise your weakest subjects but do not neglect your strongest ones. Regular revision is more effective than last-minute cramming.

---

*Last updated: 24 July 2026*

*Written by Wyatt. For questions or feedback, visit [wyattau.com](https://wyattau.com).*
