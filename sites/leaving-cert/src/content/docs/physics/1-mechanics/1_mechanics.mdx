...
description: "Mechanics is the study of motion and forces. It forms the largest single topic on the Leaving Certificate Physics paper and covers kinematics, dynamics,"
date: 2026-04-14
tags:
  - ilc
  - ilc-physics
...
1. A $4 \mathrm{ kg$ object on a frictionless surface is connected by a string over a pulley to a
    $6 \mathrm{ kg$ object hanging vertically. Find the acceleration and the tension in the string.

2. A ball is thrown vertically upward with speed $18 \mathrm{ m/s$ from a height of $5 \mathrm{ m$
    above the ground. Find the speed just before it hits the ground.

3. A satellite of mass $800 \mathrm{ kg$ orbits at $400 \mathrm{ km$ altitude. Calculate the
    orbital speed, period, and gravitational potential energy.

4. A $0.15 \mathrm{ kg$ mass on a spring of constant $200 \mathrm{ N/m$ has amplitude
    $0.05 \mathrm{ m$. Calculate the period, maximum speed, maximum KE, and total energy.

5. A car of mass $1200 \mathrm{ kg$ travels around a banked curve of radius $80 \mathrm{ m$ at
    $18 \mathrm{ m/s$. The banking angle is $15^{\circ}$. Find the minimum coefficient of static
    friction required.

6. Two ice skaters, one of mass $60 \mathrm{ kg$ and one of mass $40 \mathrm{ kg$Push apart. The
...
7. A block of mass $5 \mathrm{ kg$ slides $6 \mathrm{ m$ down a $35^{\circ}$ incline with
    $\mu_k = 0.15$. Find the work done by friction, the work done by gravity, and the final speed if
    the block started from rest.

8. A spring of spring constant $150 \mathrm{ N/m$ is compressed by $0.12 \mathrm{ m$ and launches a
    $0.3 \mathrm{ kg$ block on a frictionless surface. Calculate the speed of the block as it leaves
    the spring.

9. Explain why a satellite in a circular orbit around Earth is not truly weightless. Include
    reference to the gravitational field strength at orbital altitude.

10. A $2 \mathrm{ kg$ object is projected up a $30^{\circ}$ rough incline at $10 \mathrm{ m/s$ with
...
:::note
<strong>The study of projectile motion with air resistance requires numerical methods or</strong>
...
1. A $500 \mathrm{ g$ ball is attached to a string of length $1.2 \mathrm{ m$ and whirled in a
...
2. Two objects of mass $4 \mathrm{ kg$ and $6 \mathrm{ kg$ collide. The $4 \mathrm{ kg$ object is
...
3. A lift of mass $800 \mathrm{ kg$ accelerates upward at $2.5 \mathrm{ m/s^2$ for 3 s, then
...
4. A spring of natural length $0.3 \mathrm{ m$ and spring constant $200 \mathrm{ N/m$ is compressed
...
5. Derive an expression for the acceleration of an object sliding down a frictionless incline of
...
- **[Light and Waves](../2-waves/2_waves.md):** Covers wave properties, optics, and the electromagnetic spectrum, complementing mechanics with wave phenomena.
- **[Electricity](../3-electricity/3_electricity.mdx):** Explores electric circuits and electromagnetic induction, connecting force and energy concepts.
- **[Heat and Thermodynamics](../4-heat/4_heat.md):** Covers thermal physics and energy transfer, extending the energy conservation principles from mechanics.
- **[Practice Physics](../practice-physics.mdx):** Interactive practice problems covering mechanics, waves, electricity, and modern physics.


```mermaid
flowchart TD
    A[1_Mechanicsx] --> B[Key Concepts]
    A --> C[Core Principles]
...
Mechanics is the physics of **motion and its causes**. The core idea is Newton's second law: a net force causes acceleration proportional to mass. Everything else — kinematics equations, friction, energy, momentum, circular motion — flows from this single principle combined with careful vector resolution. The most important skill in mechanics is drawing correct free body diagrams: identify the object, draw all forces acting _on_ it, resolve forces into components, and apply $F = ma$ in each direction separately.

Energy conservation is the most powerful problem-solving tool in mechanics because it is **path-independent**. You do not need to know the details of the motion — only the initial and final states. If a roller coaster starts at height $h$ and ends at height 0, conservation of energy tells you its final speed regardless of the track shape. When friction is present, the work-energy theorem extends this: the work done by non-conservative forces equals the change in total mechanical energy. This approach often avoids the need for calculus entirely.

Momentum conservation is the tool for **collision problems**. The total momentum before a collision equals the total after, provided no external forces act. In elastic collisions, kinetic energy is also conserved. In inelastic collisions (like objects sticking together), kinetic energy is lost — converted to heat, sound, or deformation. The impulse-momentum theorem ($F\Delta t = \Delta p$) explains why car crumple zones work: by increasing the collision time, they reduce the average force on the passenger, even though the change in momentum is the same.
