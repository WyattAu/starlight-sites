...
description: "This topic covers electric charge, current, potential difference, resistance, circuits, and Electromagnetic induction. Electricity is one of the core areas"
date: 2026-04-14
tags:
  - ilc
  - ilc-physics
...
1. A battery of EMF $9\mathrm{ V$ and internal resistance $1.5\mathrm{ \Omega$ is connected to a
...
2. A $47\mu\mathrm{F$ capacitor is charged to $20\mathrm{ V$ and then discharged through a
...
3. An AC circuit has $R = 150\mathrm{ \Omega$$L = 0.3\mathrm{ H$$C = 30\mathrm{ \mu\mathrm{F$ at
    $f = 200\mathrm{ Hz$. Calculate $X_L$$X_C$$Z$The phase angle, the current, and the power
    dissipated.

4. A transformer with 1000 primary turns and 50 secondary turns is connected to a
...
5. Explain the difference between a capacitor and an inductor in an AC circuit. Include a
    description of how the impedance of each changes with frequency.

6. A potential divider circuit consists of a $20\mathrm{ k\Omega$ resistor and an NTC thermistor in
...
7. Two capacitors of $33\mu\mathrm{F$ and $100\mu\mathrm{F$ are connected in series and then in
...
8. Describe Faraday's law of electromagnetic induction. Explain Lenz's law and why it is a
    consequence of conservation of energy. Include a description of one experimental demonstration.

9. A coil of 150 turns, area $0.05\mathrm{ m^2$Resistance $5\mathrm{ \Omega$Is in a magnetic field
    of flux density $0.6\mathrm{ T$. The field is reversed in $0.02\mathrm{ s$. Calculate the
    average EMF induced.

10. Explain why the national grid uses step-up and step-down transformers. Include a quantitative
...
:::note
<strong>Only resistors dissipate power in AC circuits. Inductors and capacitors store and release</strong>
...
:::note
<strong>Useful rules of thumb for RC circuits:</strong>
...
1. A coil of inductance $0.2 \mathrm{ H$ and resistance $10 \Omega$ is connected to a
...
2. A $6 \mathrm{ V$ battery with internal resistance $0.5 \Omega$ is connected to two parallel
...
3. An AC generator produces an EMF given by $\varepsilon = 170\sin(314t)$ V. Calculate (a) the peak
    voltage, (b) the RMS voltage, (c) the frequency, and (d) the peak voltage across a $50 \Omega$
    resistor connected to the generator.

4. A capacitor of $22 \mu\mathrm{F$ is charged to $50 \mathrm{ V$ and then connected across an
...
5. Describe an experiment to determine the relationship between the current through a filament lamp
...
- **[Mechanics](../1-mechanics/1_mechanics.mdx):** Covers work, energy, and power concepts that connect to electrical energy and circuits.
- **[Light and Waves](../2-waves/2_waves.md):** Explores the electromagnetic spectrum and wave properties, linking to electromagnetic induction.
- **[Heat and Thermodynamics](../4-heat/4_heat.md):** Covers energy transfer and thermodynamics, connecting to the energy dissipation in electrical circuits.
- **[Practice Physics](../practice-physics.mdx):** Interactive practice problems covering electricity, circuits, and other physics topics.


```mermaid
flowchart TD
    A[3_Electricityx] --> B[Key Concepts]
    A --> C[Core Principles]
...
Electricity is the physics of **charge in motion**. The three fundamental quantities — voltage (energy per unit charge), current (rate of charge flow), and resistance (opposition to flow) — are related by Ohm's law: $V = IR$. Think of a circuit as a water system: voltage is the pump pressure, current is the flow rate, and resistance is the pipe narrowness. This analogy extends logically: resistors in series are like pipes in a chain (resistances add), while resistors in parallel are like parallel pipes (total resistance decreases).

Capacitors store energy in electric fields and behave very differently from resistors. A capacitor resists _changes_ in voltage — it charges exponentially when connected to a supply and discharges exponentially when the supply is removed. The time constant $\tau = RC$ determines how fast this happens: after one $\tau$, a capacitor reaches 63.2% of its final charge; after $5\tau$, it is effectively fully charged. This exponential behaviour is why capacitors are used for timing circuits, smoothing power supply output, and filtering signals.

Electromagnetic induction is the principle behind generators and transformers. Faraday's law tells you that a changing magnetic flux through a coil induces an EMF, and Lenz's law tells you the induced current always opposes the change that caused it. Transformers exploit this to step voltage up or down: stepping up to high voltage for efficient long-distance transmission (reducing $I^2R$ losses), then stepping down to safe levels for domestic use. The relationship $V_s/V_p = N_s/N_p$ connects the voltage ratio to the turns ratio, and energy conservation requires $V_p I_p = V_s I_s$ for an ideal transformer.
