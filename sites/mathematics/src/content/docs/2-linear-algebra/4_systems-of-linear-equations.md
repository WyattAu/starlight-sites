...
- **[Matrices](/2-linear-algebra/3_matrices/)**: The coefficient matrix of a linear system determines its solvability via rank and determinant.
- **[Linear Transformations](/2-linear-algebra/6_linear-transformations/)**: A system $A\mathbf{x} = \mathbf{b}$ can be viewed as finding the preimage of $\mathbf{b}$ under the linear transformation defined by $A$.
- **[Singular Value Decomposition](/2-linear-algebra/8_singular-value-decomposition/)**: The pseudoinverse derived from the SVD provides the minimum-norm least squares solution to inconsistent systems.
