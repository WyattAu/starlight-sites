...
tags:
  - Mathematics
  - University
description: "Second-Order Linear ODEs: comprehensive educational content notes with precise definitions, worked examples, common pitfalls, and practice problems."
---
...
A mass $m$ on a spring with spring constant $k$ and damping coefficient $c$, subject to external
force $F(t)$, satisfies
...
### 3.17 Intuition: What Does the Characteristic Equation Tell You?

The characteristic equation $r^2 + ar + b = 0$ encodes the **natural behavior** of the system
without external forcing. The roots $r_1, r_2$ determine whether the system oscillates, decays, or
grows:

- **Distinct real roots $r_1 < r_2 < 0$:** Both solutions decay exponentially. The system is
  **overdamped**: the damping is so strong that the mass returns to equilibrium without oscillating.
  The slower decay rate $r_2$ dominates for large $t$.

- **Repeated real root $r < 0$:** The solution is $(c_1 + c_2 t)e^{rt}$. The factor $t e^{rt}$
  creates a transient "bump" before decay dominates. This is **critical damping**: the fastest
  return to equilibrium without oscillation.

- **Complex roots $r = \alpha \pm i\beta$:** The solution is $e^{\alpha t}(c_1 \cos \beta t +
  c_2 \sin \beta t)$. The system oscillates at frequency $\beta$ with amplitude decaying as
  $e^{\alpha t}$. This is **underdamped**: the mass oscillates around equilibrium with decreasing
  amplitude.

- **Pure imaginary roots $r = \pm i\beta$:** The solution is $c_1 \cos \beta t + c_2 \sin \beta t$.
  The system oscillates forever with constant amplitude. This is **undamped** (no friction).

- **Roots with positive real part:** The solution grows without bound. The system is **unstable**.

**Physical examples:**
- A car suspension is designed to be slightly underdamped: it absorbs bumps quickly but may
  oscillate once or twice before settling.
- A door closer is in standard practice critically damped: it shuts the door as fast as possible without
  slamming.
- A guitar string is undamped (approximately): it oscillates at its natural frequency for a long
  time.
...
<strong>Common Pitfall When using undetermined coefficients, always check whether your guess</strong>
...
<aside class="starlight-aside starlight-aside--caution">
<strong>Common Pitfall For Euler-Cauchy equations, the substitution $y = x^r$ only works for</strong>

$x > 0$. For $x < 0$Substitute $x = -e^t$ or use $y = (-x)^r$.
</aside>
<aside class="starlight-aside starlight-aside--caution">
<strong>Common Pitfall Variation of parameters always works but can lead to difficult integrals.</strong>
...
- **[First-Order ODEs](5-ordinary-differential-equations/2_first-order-odes.md)**: The integrating factor method and substitution techniques for first-order ODEs are prerequisites for understanding variation of parameters.
- **[Systems of ODEs](5-ordinary-differential-equations/4_systems-of-odes.mdx)**: Second-order equations can be converted to first-order systems, connecting the characteristic equation to eigenvalue analysis.
- **[Laplace Transforms](5-ordinary-differential-equations/5_laplace-transforms.md)**: The Laplace transform provides an alternative to undetermined coefficients for solving nonhomogeneous equations.
- **[Fourier Series](5-ordinary-differential-equations/7_fourier-series.md)**: The resonance phenomena studied here reappear when Fourier series are used to solve forced oscillation problems.
...
**Forgetting to check for repeated roots:** When the characteristic equation has a repeated root $r$, the homogeneous solution is $y = (c_1 + c_2 x)e^{rx}$, not $c_1 e^{rx} + c_2 e^{rx}$. The second solution must include the factor $x$ to be linearly independent. Students often forget this and get an incomplete solution.

**Confusing underdamped, critically damped, and overdamped:** The discriminant $b^2 - 4mk$ determines the damping type. If $b^2 - 4mk > 0$, the system is overdamped (two real roots, no oscillation). If $b^2 - 4mk = 0$, it's critically damped (one repeated real root). If $b^2 - 4mk < 0$, it's underdamped (complex roots, oscillation with decay). Students often mix up the conditions.

**Using undetermined coefficients for inappropriate equations:** The method of undetermined coefficients only works when the forcing term is a polynomial, exponential, sine, cosine, or products of these. For other forcing terms (like $\tan x$ or $\ln x$), you must use variation of parameters. Students often try undetermined coefficients and get stuck.

**Forgetting to multiply by $t$ in the resonant case:** When the forcing term matches a homogeneous solution (resonance), the particular solution must include a factor of $t$. For example, if $\cos(\omega t)$ is already a solution, guess $y_p = At\sin(\omega t) + Bt\cos(\omega t)$. Forgetting the $t$ gives an incorrect solution.

**Confusing the Wronskian with the determinant:** The Wronskian $W(y_1, y_2) = y_1 y_2' - y_1' y_2$ is a specific determinant, but not all determinants are Wronskians. Students often compute the wrong determinant or forget that $W = 0$ indicates linear dependence.