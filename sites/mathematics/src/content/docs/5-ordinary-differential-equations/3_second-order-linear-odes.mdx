...
<Aside type="caution" title="Common Pitfall When using undetermined coefficients, always check whether your guess">
...
<Aside type="caution" title="Common Pitfall For Euler-Cauchy equations, the substitution $y = x^r$ only works for">
$x > 0$. For $x < 0$Substitute $x = -e^t$ or use $y = (-x)^r$.
</Aside>
<Aside type="caution" title="Common Pitfall Variation of parameters always works but can lead to difficult integrals.">
