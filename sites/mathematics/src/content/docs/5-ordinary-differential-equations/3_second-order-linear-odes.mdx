...
<Aside type="caution">
Common Pitfall For Euler-Cauchy equations, the substitution $y = x^r$ only works for

$x > 0$. For $x < 0$Substitute $x = -e^t$ or use $y = (-x)^r$.
</Aside>

<Aside type="caution">
Common Pitfall Variation of parameters always works but can lead to difficult integrals.
...
- **[First-Order ODEs](/5-ordinary-differential-equations/2_first-order-odes/)**: The integrating factor method and substitution techniques for first-order ODEs are prerequisites for understanding variation of parameters.
- **[Systems of ODEs](/5-ordinary-differential-equations/4_systems-of-odes/)**: Second-order equations can be converted to first-order systems, connecting the characteristic equation to eigenvalue analysis.
- **[Laplace Transforms](/5-ordinary-differential-equations/5_laplace-transforms/)**: The Laplace transform provides an alternative to undetermined coefficients for solving nonhomogeneous equations.
- **[Fourier Series](/5-ordinary-differential-equations/7_fourier-series/)**: The resonance phenomena studied here reappear when Fourier series are used to solve forced oscillation problems.
