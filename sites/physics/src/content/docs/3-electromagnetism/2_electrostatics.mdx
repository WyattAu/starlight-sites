...
<script type="application/ld+json">
{
  ...
  "@type": "BreadcrumbList",
  "itemListElement": [{"name": "Home", "url": "https://wyattau.com"}, {"name": "physics", "url": "https://physics.wyattau.com"}, {"name": "3 Electromagnetism", "url": "https://physics.wyattau.com/3-electromagnetism"}, {"name": "2_electrostatics", "url": "https://physics.wyattau.com/3-electromagnetism/2_electrostatics"}]
}
</script>
...
tags:
  - Physics
  - University
description: "The force between two point charges and separated by distance : Comprehensive educational content coverage with definitions and practice problems."
---
...
<DesmosGraph client:only="solid-js" title="Electric Field: 1/r^2 Dependence" expressions={["\\frac{k}{(x-d)^2}",
...
**Mistake 1: Using Gauss's law when the symmetry is insufficient**
Gauss's law is always true, but it is only useful for computing $\mathbf{E}$ when the charge distribution has sufficient symmetry (spherical, cylindrical, or planar) to pull $E$ out of the integral. Applying it to asymmetric distributions without the symmetry argument gives an integral equation that cannot be solved analytically.

**Mistake 2: Confusing the potential of a point charge with the potential energy**
The electric potential $V = q/(4\pi\varepsilon_0 r)$ is the potential energy per unit charge, not the total energy. To find the energy of a system of charges, you must compute $U = \frac{1}{2}\sum_i q_i V_i$ or equivalently $U = \frac{1}{2}\int \rho V\, d^3\mathbf{r}$. The factor of $\frac{1}{2}$ avoids double-counting.

**Mistake 3: Assuming the electric field is continuous across a charged surface**
The normal component of $\mathbf{E}$ is discontinuous across a surface charge density $\sigma$: $E_{2n} - E_{1n} = \sigma/\varepsilon_0$. Only the tangential component $E_t$ is continuous. Students who assume full continuity obtain incorrect boundary conditions for dielectric interfaces.

## Cross-References

- **[Maxwell's Equations](1_maxwell-s-equations.md)**: Electrostatics provides the static electric field solutions that form two of Maxwell's four equations.
- **[Potentials and Gauge Transformations](6_potentials-and-gauge-transformations.md)**: The electric potential introduced here is the scalar potential used in the general gauge theory framework.
- **[Electrodynamics](4_electrodynamics.md)**: Electrostatics is the zero-velocity limit of the full electrodynamics theory developed in the next chapter.
