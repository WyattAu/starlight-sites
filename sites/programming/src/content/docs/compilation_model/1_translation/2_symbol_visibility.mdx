...
<Aside type="danger" title="The ODR Violation Trap">
If two different TUs define the same class/struct `Foo` (e.g., via copy-pasted headers), but the
definitions differ (e.g., different member order or types), the linker **may not detect this**. This
is **Undefined Behavior**. The runtime may crash or corrupt memory, because code in TU `A` assumes
one memory layout while code in TU `B` assumes another.
</Aside>
...
```mermaid
flowchart TD
    A[2_Symbol_Visibilityx] --> B[Key Concepts]
    A --> C[Core Principles]
...
Symbol visibility is like deciding which conversations happen in public and which happen behind closed doors. When a function is exported, it is available to any other translation unit that links against it, like a public square where anyone can listen. When it is hidden, it is a private conversation between functions in the same file. Controlling visibility reduces compilation dependencies and prevents name collisions, the way a building's internal corridors keep private offices separate from the public lobby.
...
- [1_translation](./)
- [Preprocessing and AST Generation, and Object Code](/compilation_model/1_translation/1_preprocessing_ast_object/)
