...
Binary formats are the structured containers that the operating system loader understands. They organise compiled code, data, and metadata into sections with specific memory permissions, enabling the OS to map them efficiently into virtual memory. ELF on Linux, PE on Windows, and Mach-O on macOS each solve the same problem differently, but all separate executable code from read-write data and include symbol tables for linking and debugging. Understanding these formats explains why stripped binaries are smaller, how dynamic linking resolves symbols at load time, and why position-independent code is necessary for shared libraries.
