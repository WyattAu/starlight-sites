...
title: "Name Mangling | Programming - Wyatt's Notes"
...
1. **Check the library/object file** that should provide the symbol:
   ...
   ```
2. **If the symbol exists but the linker cannot find it**, check link order (libraries after object
...
```mermaid
flowchart TD
    A[5_Name_Manglingx] --> B[Key Concepts]
    A --> C[Core Principles]
