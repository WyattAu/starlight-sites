...
title: "Code Coverage | Programming - Wyatt's Notes"
description: "Code coverage is a metric used to calculate the percentage of source code executed during the Testing phase. In high-reliability C++ systems, coverage"
...
:::caution
<strong>Performance Overhead Instrumentation significantly increases binary size and execution</strong>
...
- [Unit Tests](5_unit_tests) — Writing tests that drive coverage analysis
- [Build Caching](4_build_caching) — Avoiding cache poisoning with instrumented builds
- [CMake Targets and Properties](1_cmake_targets_properties_generator) — Custom targets for
  coverage automation


```mermaid
flowchart TD
    A[6_Code_Coveragex] --> B[Key Concepts]
    A --> C[Core Principles]
...
Code coverage measures which lines of code are executed during testing. gcov and llvm-cov collect coverage data, while lcov and gcovr generate reports. Coverage thresholds enforce minimum testing standards. Integration with CI pipelines ensures coverage does not regress. Combined with mutation testing, coverage metrics provide insight into test quality beyond simple line execution.

## Cross-References

- [[enviroment_and_toolchain/2_build_system/5_unit_tests]] - Test execution for coverage
- [[enviroment_and_toolchain/2_build_system/4_build_caching.mdx]] - Coverage and build caching
- [[enviroment_and_toolchain/2_build_system/2_ninja_and_parallelism]] - Parallel coverage collection
- [[enviroment_and_toolchain/4_development_enviroment_analysis/3_static_analysis.mdx]] - Static analysis complement
