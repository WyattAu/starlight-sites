...
Fundamental types are the alphabet of the machine, each one maps directly to a register or a specific instruction set. An `int` is not an abstract concept; it is a specific number of bits that the CPU knows how to add, subtract, and compare in a single cycle. The data model (LP64 vs LLP64) is like different countries using different paper sizes, the same type has different sizes on different platforms, which is why you cannot assume `long` is always 8 bytes. Endianness is about which end of the number you read first, like reading a number left-to-right versus right-to-left. These details matter when data crosses system boundaries: a file written on one machine may be garbled on another if the byte order is different.
...
```mermaid
flowchart TD
    A[1_Fundamental_Typesx] --> B[Key Concepts]
    A --> C[Core Principles]
...
- [1_data_layout](./)
- [Alignment and Layout](/types/1_data_layout/2_layout/)
- [Bit Manipulation](/types/1_data_layout/3_bit_manipulation/)
...
4. IEEE (2019). *IEEE Standard for Floating-Point Arithmetic* (IEEE 754-2019). Institute of Electrical and Electronics Engineers.
