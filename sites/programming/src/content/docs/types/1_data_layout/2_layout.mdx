...
<script type="application/ld+json">
{
  ...
  "@type": "BreadcrumbList",
  "itemListElement": [{"name": "Home", "url": "https://wyattau.com"}, {"name": "programming", "url": "https://programming.wyattau.com"}, {"name": "Types", "url": "https://programming.wyattau.com/types"}, {"name": "1_data_layout", "url": "https://programming.wyattau.com/types/1_data_layout"}, {"name": "2_layout", "url": "https://programming.wyattau.com/types/1_data_layout/2_layout"}]
}
</script>
...
Alignment is like packing a suitcase — you cannot just throw items in randomly; some items must be placed in specific positions to fit properly. The compiler inserts padding bytes between struct members to ensure each member starts at an address that is a multiple of its alignment requirement. This wastes memory but enables the CPU to read data in a single instruction. Without alignment, the CPU might need two memory reads to fetch a single value that straddles a cache line boundary — like needing to open two doors to walk through one doorway. The `alignas` and `alignof` keywords give you explicit control over this packing process.
