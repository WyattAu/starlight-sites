...
title: Stack
description: "Stack memory allocation in C++."
...
<script type="application/ld+json">
{
  ...
  "@type": "BreadcrumbList",
  "itemListElement": [{"name": "Home", "url": "https://wyattau.com"}, {"name": "programming", "url": "https://programming.wyattau.com"}, {"name": "Types", "url": "https://programming.wyattau.com/types"}, {"name": "1_data_layout", "url": "https://programming.wyattau.com/types/1_data_layout"}, {"name": "4_stack", "url": "https://programming.wyattau.com/types/1_data_layout/4_stack"}]
}
</script>
...
The stack is like a stack of plates in a cafeteria — you can only add or remove from the top, and you do it in constant time. When a function is called, a new plate is placed on top with all its local variables; when it returns, that plate is removed. This is why stack allocation is so fast — it is just moving a pointer. But the stack has a fixed size (in most cases a few megabytes), so you cannot put infinitely many plates on it — that is a stack overflow. The heap is like a warehouse with unlimited space, but finding and claiming space there takes more effort because the warehouse manager must search for a free spot.
