...
Reference lifetime rules prevent you from using a reference after the object it refers to has been destroyed, the way a library reference card is useless after the book has been removed from the shelf. Temporary lifetime extension binds a reference to a temporary, keeping it alive as long as the reference exists. Understanding these rules prevents dangling references, which are bugs where you are reading mail that has already been forwarded to a new address.
