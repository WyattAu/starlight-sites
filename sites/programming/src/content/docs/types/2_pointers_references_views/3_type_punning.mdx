...
Type punning is the act of looking at the same memory through a different type lens, like examining a painting through a magnifying glass versus a wide-angle lens. The underlying bytes have not changed, but your interpretation of them has. `std::bit_cast` and `std::memcpy` are the safe ways to do this, while reinterpret_casting is like reading a binary file as text: it might work, but you are violating the file's intended format and the compiler may optimize away your assumptions.
