...
description: "`constexpr` tells the compiler \"this value can be computed at compile time if all inputs are known,\" turning runtime work into compile-time work, like"
...
```mermaid
flowchart TD
    A[4_Constexpr_Consteval_Constinitx] --> B[Key Concepts]
    A --> C[Core Principles]
