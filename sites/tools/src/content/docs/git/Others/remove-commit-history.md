...
- [Filter Repo](/git/05-advanced-topics/10-filter-repo/) is the modern recommended tool for the history rewriting operations this guide covers.
- [Git Objects](/git/02-fundamentals/02-git-objects/) explains the object model that must be reconstructed when commit history is removed.
- [Reflog](/git/05-advanced-topics/01-reflog/) shows how to recover from accidental history rewrites using the reference log.
