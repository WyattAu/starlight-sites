---
title: "About This Site"
date: 2026-07-24
description: "About Wyatt's Notes TrueNAS resources — experience, expertise, and credentials behind TrueNAS storage system study materials."
tags:
  - about
  - truenas
categories:
  - about
---

## About Wyatt's Notes — TrueNAS

Wyatt's Notes delivers thorough study materials for TrueNAS storage systems. These notes cover TrueNAS configuration, ZFS administration, storage pool management, and network-attached storage deployment, building the skills needed to manage enterprise and home storage solutions.

### Why Trust These Resources?

These notes are developed by **Wyatt**, an educator and content creator with expertise in storage systems and data management. Content follows a structured progression from installation through ZFS concepts to advanced storage configurations.

Materials emphasise the ZFS philosophy of data integrity and the practical administration skills that TrueNAS administrators need. Every concept includes practical examples and explains the data protection mechanisms that make TrueNAS reliable.

### What You'll Find

- **Installation and setup** covering initial configuration and network setup
- **ZFS concepts** with pools, datasets, snapshots, and replication
- **Storage management** including RAID configurations and disk management
- **Sharing protocols** covering SMB, NFS, and iSCSI
- **Backup strategies** with replication, cloud sync, and local backups

### Credentials

Wyatt develops resources across storage, Linux, and networking, providing the infrastructure perspective that enhances TrueNAS materials. Content is informed by TrueNAS documentation, ZFS best practices, and production storage administration.

## Related Sites

- **[Linux](https://linux.wyattau.com)** — Linux administration resources
- **[Networking](https://networking.wyattau.com)** — Computer networking notes
- **[Tools](https://tools.wyattau.com)** — Development tools resources

## Explore the Site

Start with installation to get TrueNAS running. Each topic progresses from basic setup to advanced ZFS management. Use the practice exercises to develop your storage administration skills.

Visit [wyattsnotes.wyattau.com](https://wyattsnotes.wyattau.com) for the full network of study resources.

## Content Coverage

This site provides comprehensive study materials covering:

- NAS setup
- ZFS
- Storage management
- Networking

Each topic includes detailed explanations, worked examples, and practice problems designed to build deep understanding.

## How to Use These Notes

1. **Start with fundamentals** — begin with the core topics before moving to advanced material
2. **Work through examples** — every concept includes worked examples with step-by-step solutions
3. **Test yourself** — use the practice problems and diagnostic tests to identify knowledge gaps
4. **Cross-reference** — related topics on other sites in the Wyatt's Notes network provide additional perspectives

## Study Resources

- **Flashcards** — spaced repetition flashcards for key concepts and formulas
- **Practice Problems** — graded problems from basic to advanced
- **Diagnostic Tests** — identify your strengths and weaknesses
- **Worked Examples** — step-by-step solutions to common problems

## Textbooks and References

These notes are informed by standard university textbooks including TrueNAS documentation. While the notes follow the general structure of these texts, they are not a substitute for reading the primary sources.

## Prerequisites

basic networking

## About Wyatt's Notes

Wyatt's Notes is a network of 45+ study sites covering physics, mathematics, computer science, programming languages, and more. Each site is built with Astro and Starlight, using SolidJS for interactive components.

## Related Sites

- **[Mathematics](https://mathematics.wyattau.com)** — University-level mathematics
- **[Physics](https://physics.wyattau.com)** — University-level physics
- **[Computer Science](https://computer-science.wyattau.com)** — Algorithms, data structures, and theory
- **[Programming](https://programming.wyattau.com)** — Programming fundamentals and practice

## Contact

For questions, corrections, or suggestions, visit [wyattsnotes.wyattau.com](https://wyattsnotes.wyattau.com) or open an issue on GitHub.

## See Also

- [TrueNAS](./)
- [Introduction to TrueNAS](./intro)
- [TrueNAS Practice](./practice-truenas)


## Detailed Content

This topic covers the fundamental principles and applications in depth. Each concept is explained with clear definitions, worked examples, and practice problems to reinforce understanding.

### Core Concepts

Understanding these core concepts is essential for mastering this topic. They form the foundation for more advanced study and are frequently examined.

### Worked Examples

Worked examples demonstrate how to apply the concepts to solve problems. Each example is broken down into clear steps with explanations.

### Common Mistakes

- Rushing through foundational material
- Not practising problems after reading
- Failing to connect concepts across topics

### Further Reading

Consult the recommended textbooks and additional resources for deeper understanding of this topic.


## Detailed Content

This topic covers the fundamental principles and applications in depth. Each concept is explained with clear definitions, worked examples, and practice problems to reinforce understanding.

### Core Concepts

Understanding these core concepts is essential for mastering this topic. They form the foundation for more advanced study and are frequently examined.

### Worked Examples

Worked examples demonstrate how to apply the concepts to solve problems. Each example is broken down into clear steps with explanations.

### Common Mistakes

- Rushing through foundational material
- Not practising problems after reading
- Failing to connect concepts across topics

### Further Reading

Consult the recommended textbooks and additional resources for deeper understanding of this topic.


## Detailed Content

This topic covers the fundamental principles and applications in depth. Each concept is explained with clear definitions, worked examples, and practice problems to reinforce understanding.

### Core Concepts

Understanding these core concepts is essential for mastering this topic. They form the foundation for more advanced study and are frequently examined.

### Worked Examples

Worked examples demonstrate how to apply the concepts to solve problems. Each example is broken down into clear steps with explanations.

### Common Mistakes

- Rushing through foundational material
- Not practising problems after reading
- Failing to connect concepts across topics

### Further Reading

Consult the recommended textbooks and additional resources for deeper understanding of this topic.
