...
describe('Monorepo Structure', () => {
  it('should have all 9 expected site directories', () => {
    const actual = fs.readdirSync(SITES_DIR).filter(f => {
      return fs.statSync(path.join(SITES_DIR, f)).isDirectory()
        && f !== 'node_modules'
        && f !== 'main'
        && f !== 'built';
    });
  ...
  for (const site of SITES_WITH_MERMAID) {
    it(`${site} should have mermaid dependencies`, () => {
      const pkg = JSON.parse(
        fs.readFileSync(path.join(SITES_DIR, site, 'package.json'), 'utf8')
      );
    ...
    it(`${site} should import mermaidNoRocketLoader`, () => {
      const config = fs.readFileSync(
        path.join(SITES_DIR, site, 'astro.config.mjs'),
        'utf8'
      );
...
describe('CI/CD Configuration', () => {
  ...
  it('deploy.yml should not have hardcoded secrets', () => {
    ...
    assert.ok(!deploy.includes('b1c8e9c031cd404da7e8688c332d4374'),
      'deploy.yml should not have hardcoded KV namespace ID');
    assert.ok(!deploy.includes('26966ba2f4b3a12cb750cd615c8d0bcf'),
...
describe('Search API', () => {
  ...
  it('should not have hardcoded secrets in upload script', () => {
    ...
    assert.ok(!upload.includes('26966ba2f4b3a12cb750cd615c8d0bcf'),
