...
describe('isErrorScope', () => {
  it('excludes content pages under sites/<site>/src/content/', () => {
    ...
    assert.strictEqual(isErrorScope(path.join(ROOT, 'sites/ib/src/content/docs/foo.mdx')), false)
  })
  ...
  it('includes site data files (not content)', () => {
    assert.strictEqual(isErrorScope(path.join(ROOT, 'sites/dse/src/data/flashcards/x.js')), true)
  })
})
