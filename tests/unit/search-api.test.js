...
const TEST_INDEX = {
  entries: [
    { title: 'Quadratic Equations', url: 'https://dse.wyattau.com/maths/quadratics/', site: 'dse', content: 'A quadratic equation has the form ax^2 + bx + c = 0. Discriminant and roots.' },
    { title: 'Mechanics: Newton\'s Laws', url: 'https://dse.wyattau.com/physics/mechanics/', site: 'dse', content: 'Newton three laws. F = ma. Inertia, force, action-reaction.' },
    { title: 'Vector Spaces', url: 'https://university.wyattau.com/mathematics/linear-algebra/', site: 'university', content: 'A vector space over a field with addition and scalar multiplication axioms.' },
    { title: 'Rust Ownership', url: 'https://languages.wyattau.com/rust/ownership/', site: 'languages', content: 'Ownership borrowing lifetimes in Rust memory safety.' },
  ],
};

describe('Search Ranking', () => {
  it('ranks exact title matches above content-only matches', () => {
    const results = api.searchIndex('quadratic', TEST_INDEX, null, api.RANKING_VARIANTS.control, null, null);
  ...
  it('excludes entries with no query relevance (authority-only no longer pollutes results)', () => {
    ...
    const results = api.searchIndex('zzzznonexistent', TEST_INDEX, null, api.RANKING_VARIANTS.control, null, null);
  ...
  it('applies authority as a boost on matching entries (university > dse for a shared term)', () => {
    ...
    const results = api.searchIndex('axioms', TEST_INDEX, null, api.RANKING_VARIANTS.control, null, null);
  ...
  it('filters by site', () => {
    const results = api.searchIndex('newton', TEST_INDEX, 'dse', api.RANKING_VARIANTS.control, null, null);
    assert.ok(results.every((r) => r.site === 'dse'));
  });

  it('sorts results by score descending', () => {
    const results = api.searchIndex('rust ownership', TEST_INDEX, null, api.RANKING_VARIANTS.control, null, null);
  ...
  it('respects A/B ranking variants (variant_a weights title higher)', () => {
    const control = api.searchIndex('quadratic', TEST_INDEX, null, api.RANKING_VARIANTS.control, null, null);
    const variantA = api.searchIndex('quadratic', TEST_INDEX, null, api.RANKING_VARIANTS.variant_a, null, null);
