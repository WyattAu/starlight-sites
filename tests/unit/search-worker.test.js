  ...
  entries: [
    {
      ...
      site: 'dse',
      content: 'A quadratic equation has the form ax^2 + bx + c = 0. The discriminant determines the nature of the roots.',
      description: 'Solving quadratic equations by factorisation, the formula, and completing the square.',
    },
    {
      title: 'Mechanics: Newton\'s Laws of Motion',
      url: 'https://dse.wyattau.com/physics/mechanics/newtons-laws/',
      site: 'dse',
      content: 'Newton\'s three laws describe the relationship between a body and the forces acting on it. F = ma.',
    },
    {
      title: 'Linear Algebra: Vector Spaces',
      url: 'https://university.wyattau.com/mathematics/linear-algebra/vector-spaces/',
      site: 'university',
      content: 'A vector space over a field is a set equipped with addition and scalar multiplication satisfying eight axioms.',
    },
    {
    ...
    },
    {
      ...
      site: 'languages',
      content: 'Ownership is the central concept of Rust memory safety. Each value has a single owner.',
    },
  ],
  metadata: {
    totalEntries: 5,
...
function installMockCache() {
  ...
  const mockCache = {
    async match(_req) { return undefined; }, // bypass edge cache for deterministic tests
    async put(_req, _res) { return undefined; },
    _cached: cached,
  };
...
describe('Search Worker (mocked KV)', () => {
  ...
  describe('CORS preflight', () => {
    it('OPTIONS returns 204-style empty response with CORS headers', async () => {
      const { status, headers } = await callWorker(worker, '/api/search', { method: 'OPTIONS', env: freshEnv() });
  ...
  describe('GET /api/search', () => {
    it('rejects queries shorter than 2 characters with 400', async () => {
      const { status, body } = await callWorker(worker, '/api/search', { search: '?q=a', env: freshEnv() });
      assert.strictEqual(status, 400);
      assert.match(body.error, /at least 2 characters/i);
    });

    it('returns ranked results for a matching query', async () => {
      const { status, body } = await callWorker(worker, '/api/search', { search: '?q=quadratic', env: freshEnv() });
    ...
    it('ranks exact title matches above content-only matches', async () => {
      const { body } = await callWorker(worker, '/api/search', { search: '?q=quadratic', env: freshEnv() });
    ...
    it('filters results by site', async () => {
      const { body } = await callWorker(worker, '/api/search', { search: '?q=quadratic&site=university', env: freshEnv() });
    ...
    it('filters results by subject (physics)', async () => {
      const { body } = await callWorker(worker, '/api/search', { search: '?q=newton&subject=physics', env: freshEnv() });
      assert.ok(body.total >= 1);
    ...
    it('respects the limit parameter (capped at 50)', async () => {
      const { body } = await callWorker(worker, '/api/search', { search: '?q=quadratic&limit=1', env: freshEnv() });
    ...
    it('attaches site metadata (name, color, url) to each result', async () => {
      const { body } = await callWorker(worker, '/api/search', { search: '?q=rust', env: freshEnv() });
    ...
    it('returns 503 when the merged index is missing', async () => {
      ...
      const { status, body } = await callWorker(worker, '/api/search', { search: '?q=quadratic', env });
      assert.strictEqual(status, 503);
      assert.match(body.error, /index not available/i);
    });

    it('returns breadcrumbs derived from the URL', async () => {
      const { body } = await callWorker(worker, '/api/search', { search: '?q=vector', env: freshEnv() });
    ...
    it('includes a preview object when preview=true', async () => {
      const { body } = await callWorker(worker, '/api/search', { search: '?q=quadratic&preview=true', env: freshEnv() });
    ...
    it('returns alternative suggestions on zero results (zero-result recovery)', async () => {
      ...
      const { status, body } = await callWorker(worker, '/api/search', { search: '?q=calculus', env: freshEnv() });
      assert.strictEqual(status, 200);
      assert.strictEqual(body.total, 0, 'calculus matches no fixture entry');
      assert.ok(Array.isArray(body.suggestions), 'zero-result queries must include a suggestions array');
      assert.ok(body.suggestions.length > 0, 'calculus should yield at least one curated suggestion');
    });
  });
  ...
  describe('GET /api/sites', () => {
    it('returns the nine configured sites', async () => {
      ...
      assert.deepStrictEqual(ids, ['alevel', 'dse', 'ib', 'infrastructure', 'languages', 'programming', 'qualifications', 'tools', 'university']);
    });
  });
  ...
  describe('GET /api/suggest', () => {
    it('returns suggestions derived from titles', async () => {
      const { status, body } = await callWorker(worker, '/api/suggest', { search: '?q=quad', env: freshEnv() });
      assert.strictEqual(status, 200);
